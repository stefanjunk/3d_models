...
Before release, load the sibling `validate-printable-3d-projects` skill and apply `assets/validation-profile.json`. Hash every mold part, master, case, interface contract, slicer profile, G-code, and inspection report in `validation-project.json`. Run mesh audits, exact interface/clearance checks where available, demolding-direction motion sweeps, parameter sweeps, and G-code checks. Undercut, venting, drainability, plaster memory, shrinkage, and workshop trials remain explicit physical or review gates until an exact check exists. A required `NOT_RUN`, `REVIEW_REQUIRED`, stale report, or failed physical gate blocks release.

At project start, read the project autonomy policy before changing artifacts. Record only `AUTO_APPROVED` or `BLOCKED` in the agent ledger and only for stages assigned to the agent. Never write `HUMAN_APPROVED`; physical trials, safety, and commercial release remain in the separate human ledger. Workflow autonomy does not authorize dependency installation, upload, or printer start.
