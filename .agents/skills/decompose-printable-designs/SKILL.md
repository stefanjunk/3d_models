...
Before integration or release, load the sibling `validate-printable-3d-projects` skill and apply `assets/validation-profile.json`. Store every component's authority, datum, transform, interface contract, keep-out, source hash, derived-artifact hash, and validation owner in `validation-project.json`. Run per-part mesh audits, exact overlap/clearance checks where available, assembly motion sweeps, build parameter sweeps, and G-code/3MF checks. Generated-surface fidelity, service access, and physical interface coupons remain declared review gates. Missing required gates, stale reports, or `NOT_RUN` block release.

At project start, read the project autonomy policy before changing artifacts. Record only `AUTO_APPROVED` or `BLOCKED` in the agent ledger and only for stages assigned to the agent. Never write `HUMAN_APPROVED`; physical, safety, appearance, and commercial stages remain in the separate human ledger. Workflow autonomy does not authorize dependency installation, upload, or printer start.
