...
## Dense relief and mesh workflow

`design-spec.yaml` demonstrates separate reference/master and manufacturing mesh paths plus triangle, RAM, mesh-size, and slicer-time budgets. `mesh-simplification-metrics.json` is an illustrative passing input for the geometric relief gate; it does not replace the separate slicer layer/path report or a carbon-texture coupon.
