...
2. Read `references/watermark-release-gate.md`, copy the canonical workspace `tools/metrimade-watermark/` package into the project, and generate all exact `MM-WM-001-R2` tiers from the immutable `project.id` and `project.revision`. Never redraw a tier or type/edit its domain, product ID, or version independently.
