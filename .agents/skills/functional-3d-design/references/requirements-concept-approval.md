...
2. Load the sibling `metrimade-release-marking` skill and read its `references/watermark-release-gate.md`, copy the canonical workspace `tools/metrimade-watermark/` package into the project, and generate all exact `MM-WM-001-R2` tiers from the immutable `project.id` and `project.revision`. Never redraw a tier or type/edit its domain, product ID, or version independently.
...
4. Run that skill's `scripts/select_watermark.py` with the generated tier directory or metadata JSON files, safe-region size, host-wall thickness, nozzle, and layer height. Accept its highest-information fit in the order Full, Compact, Micro, trying 0°/90° at scale 1.0. Micro may omit the domain only through this failover. If Micro does not fit, choose another safe region or revise the product geometry.
...
Use watermark states `blocked`, `pending`, `approved`, and `changes-requested`. Record the specification revision, immutable production-geometry revision/hash, watermark asset ID, selected layout tier, domain visibility, exact product ID/version, generated profile metadata and manifest, placement, preview, digital and physical validation evidence, approving user, and timestamp.
