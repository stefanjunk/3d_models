...
- source/reference and candidate triangle count and file size;
...
Every manufacturing mesh also needs the decision gate in `mesh-simplification.md`. When simplification is attempted, compare bidirectional maximum/RMS surface error, protected-region error, bounds/volume, bed contact, minimum wall, cosmetic/relief amplitude, and exact-slicer behavior. Record `not-beneficial` instead of applying a lossy step when no meaningful handling or toolpath benefit exists.
...
For optimization candidates, retain the identical baseline profile and compare model/support material, time by feature, layer count, path length/retractions/short segments when available, peak flow, import time, and slice computation time. Do not infer slicer or print-time benefit from CAD volume or triangle count alone.
