...
Use the versioned generator package `MM-WM-001-R1`: in this workspace its canonical source is `metrimade-watermark/`; a packaged skill carries the same pinned core under `assets/metrimade-watermark/`. Verify the revision and use exactly one of these sources. Every generated production profile contains exactly two visible lines:
...
Generate the profile from the immutable release identity. Never redraw the mark, substitute live text, or edit the domain, product ID, or version independently. The bundled asset is digitally validated but remains `DIGITAL_PRODUCTION_CANDIDATE_PHYSICAL_TEST_PENDING`; a passed coupon on the intended production process is mandatory before release approval.
...
- Do not overwrite or silently rebuild historical releases carrying a JuSt Innovation mark. Introduce `MM-WM-001-R1` only through a new product revision, retain the historical release, and rerun affected checks.

## Generate the exact product profile

Copy the complete workspace `metrimade-watermark/` package, or the packaged skill's `assets/metrimade-watermark/` core, into the project as `assets/metrimade-watermark/`. Then generate the profile from `project.id` and `project.revision`:
...
Generation requires Inter Variable, OpenSCAD, ImageMagick, `fontTools`, and `trimesh`. It fails for a missing or malformed uppercase hyphenated product ID, a non-SemVer version, an unavailable dependency, or invalid output geometry. Retain the generated SVG, DXF, SCAD wrapper, mirrored STL cutter, coupon STL, PNG, metadata JSON, icon source, and `manifest.sha256` together.

The generated profile size depends on the trace line and is authoritative. Do not scale it down, crop it, distort it, reduce gaps, or omit either line. If it does not fit safely, use another surface or revise the product geometry.
...
python scripts/select_watermark.py \
  --metadata assets/metrimade-watermark/generated/MM-ORG-001_v1.0.0/metrimade-watermark-MM-ORG-001-v1.0.0.json \
  --surface-width 80 --surface-height 45 \
  --host-wall 2.0 --nozzle 0.4 --layer-height 0.2
```

The selector permits only 0° or 90° placement at scale 1.0. Default edge clearance is the larger of 2.0 mm or two nozzle diameters. Require a host wall of at least 1.20 mm and a remaining wall of at least 0.80 mm after engraving. Qualified depth is 0.40–0.80 mm; the generated default is 0.40 mm. Treat `BLOCK` as final for that candidate region.

## Integrate the exact geometry

- **OpenSCAD:** use the generated SCAD wrapper directly or copy `source/metrimade-watermark.scad`, pass the generated SVG and its metadata width, and call `metrimade_watermark_cutter()` in a `difference()`.
...
Record asset revision, fixed domain, exact product ID/version, generated metadata and manifest hashes, actual envelope, rotation, position, surface, depth, edge/feature clearances, local wall before/after, marked-part coverage, printer/nozzle/material/profile, and production-geometry revision/hash in `design-spec.yaml` or linked evidence. The watermark identity must match the release manifest, filenames/package, and catalog identity.

Show all of the following from the actual production candidate:

1. orthographic finished-underside view with readable orientation and both identity lines;
...
6. a passed generated coupon for the exact intended printer, nozzle, layer profile, material/color, first-layer settings, and bed surface.
