...
metadata:
  ...
  compatibility: "Codex/OpenCode; optional Blender, CAD and mesh tools"
---
...
If the source mesh still needs to be generated by the local Step1X service, load `step1x-image-to-3d` first. Return here only after its raw geometry/textured GLBs, scale uncertainty and run evidence have been preserved.

If this work creates a new independently managed product rather than editing a
component of an existing SKU, load `3d-design-preflight` and complete its
