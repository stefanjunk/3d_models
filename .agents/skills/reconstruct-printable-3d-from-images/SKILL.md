...
Before release, load the sibling `validate-printable-3d-projects` skill and apply `assets/validation-profile.json`. Hash every input image, camera/mask assumption, dimensions, editable source, reconstructed mesh, manufacturing mesh, slicer profile, G-code, and comparison report. Bind view-comparison reports to the exact camera, render, mask, and source-image hashes; run mesh, geometry-comparison, G-code, 3MF, and freshness checks. Hidden geometry, ambiguous scale, semantic interpretation, and critical fit remain named review or physical gates. Required `NOT_RUN`, `REVIEW_REQUIRED`, or stale evidence blocks release.

At project start, read the project autonomy policy before changing artifacts. Record only `AUTO_APPROVED` or `BLOCKED` in the agent ledger and only for stages assigned to the agent. Never write `HUMAN_APPROVED`; hidden-geometry review, physical fit, appearance, safety, and commercial stages remain in the separate human ledger. Workflow autonomy does not authorize dependency installation, upload, or printer start.
