...
Read this reference before editing or exporting a Step1X geometry GLB.

## What the GLB is

- `geometry.raw.glb`: the untextured generation result and immutable geometry master. The owned fork has no texture stage and produces no textured GLB.

Keep it immutable and hashed. Work from copies.
...
|---|---|---|
| inspection, presentation, Blender source | GLB plus optional `.blend` | preserves nodes, transforms and mesh data; no generated texture is present |
...
The owned fork supplies geometry only. Load `multicolor-fdm-design` to add colour as an independently authored manufacturing intent:

- define colour regions as explicit solids, inlays or reviewed slicer painting;
- map them to an actual measured filament palette;
...
Direct STL export carries geometry only. Preserve separate colour references and their provenance when they influence a multicolour derivative.
