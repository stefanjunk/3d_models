...
from fdm_validation.common import (  # noqa: E402
    check,
    exit_code,
    load_data,
    report,
...
from fdm_validation.doctor import MODULE_GROUPS  # noqa: E402
from fdm_validation.doctor import run as run_doctor
...
from fdm_validation.p2author import author_anycubic_3mf  # noqa: E402
from fdm_validation.p2stage import validate as validate_p2_stage  # noqa: E402
...
def parser() -> argparse.ArgumentParser:
    root = argparse.ArgumentParser(
        description="Deterministic validation CLI for printable 3D projects."
    )
    commands = root.add_subparsers(dest="command", required=True)

    item = commands.add_parser(
        "doctor",
        help="Report optional capabilities and tool versions without installing anything.",
    )
    item.add_argument(
        "--require", action="append", choices=sorted(MODULE_GROUPS), default=[]
    )
    output_args(item)

    item = commands.add_parser(
        "audit-mesh",
        help="Audit mesh topology, size, bed fit, budgets, and optional sampled thickness.",
    )
    ...
    item = commands.add_parser(
        "compare-meshes",
        help="Run seeded bidirectional surface and dimensional regression.",
    )
    ...
    item = commands.add_parser(
        "check-interfaces",
        help="Check overlap, clearance, and discretized motion contracts.",
    )
    ...
    item = commands.add_parser(
        "analyze-gcode",
        help="Parse local G-code without uploading or starting a printer.",
    )
    ...
    item = commands.add_parser(
        "slice-anycubic-next",
        help="Slice locally with Anycubic Slicer Next and record exact profile/output hashes.",
    )
    ...
    item.add_argument(
        ...
        type=int,
        default=0,
        help="0 slices all plates; positive values select one plate.",
    )
    ...
    item = commands.add_parser(
        "validate-3mf",
        help="Validate standard 3MF package, references, materials, and optional topology.",
    )
    ...
    item = commands.add_parser(
        "validate-p2-stage",
        help="Validate the four mandatory P2 artifacts and the complete oriented/support-planned 3MF print set.",
    )
    ...
    item = commands.add_parser(
        "author-anycubic-3mf",
        help="Create a non-overwriting Anycubic project 3MF from complete, already oriented sources and exact profiles.",
    )
    item.add_argument("output", type=Path)
    item.add_argument("source", nargs="+", type=Path)
    item.add_argument("--machine-profile", required=True, type=Path)
    item.add_argument("--process-profile", required=True, type=Path)
    item.add_argument("--filament-profile", required=True, type=Path)
    item.add_argument("--support-mode", required=True, choices=("disabled", "enabled"))
    item.add_argument("--slicer", default="AnycubicSlicerNext")
    item.add_argument("--timeout-s", type=int, default=600)
    output_args(item)

    item = commands.add_parser(
        "run-sweep", help="Execute a declared default/min/max/pairwise parameter sweep."
    )
    item.add_argument("manifest", type=Path)
    ...
    item = commands.add_parser(
        "validate-skill", help="Check skill layout and scripts without bytecode writes."
    )
    ...
    item = commands.add_parser(
        "validate-profile",
        help="Validate a companion skill's deterministic validation profile.",
    )
    ...
    item = commands.add_parser(
        "init-autonomy",
        help="Create a project-scoped autonomy policy without overwriting an existing file.",
    )
    item.add_argument("project_id")
    item.add_argument("output", type=Path)
    item.add_argument(
        "--mode",
        choices=("manual", "guided", "autonomous-to-print-candidate", "custom"),
        default="autonomous-to-print-candidate",
    )
    item.add_argument(
        "--authorized-by",
        required=True,
        help="Human identity that selected this workflow delegation.",
    )
    item.add_argument(
        "--preflight",
        type=Path,
        help="Bind autonomy and its maximum mode to this validated preflight result.",
    )
    ...
    item = commands.add_parser(
        "validate-autonomy",
        help="Validate stage authorities, evidence rules, and the tool-permission boundary.",
    )
    ...
    item = commands.add_parser(
        "approve-agent-stage",
        help="Derive AUTO_APPROVED or BLOCKED from the policy and hash-bound evidence.",
    )
    item.add_argument("policy_file", type=Path)
    ...
    item = commands.add_parser(
        ...
        help="Freeze a hash-bound request for a human-controlled stage.",
    )
    item.add_argument("policy_file", type=Path)
    item.add_argument("stage")
    item.add_argument("project_id")
    ...
    item = commands.add_parser(
        "approve-human-stage",
        help="Record HUMAN_APPROVED in the separate human ledger; optionally require HMAC proof.",
    )
    item.add_argument("policy_file", type=Path)
    ...
    item = commands.add_parser(
        "validate-approvals",
        help="Validate ledger chains, evidence hashes, actors, and all stages through a target.",
    )
    item.add_argument("policy_file", type=Path)
    ...
    item = commands.add_parser(
        "freeze-project",
        help="Write a non-destructive project copy with current artifact SHA-256 hashes.",
    )
    ...
    item = commands.add_parser(
        "validate-project",
        help="Execute and aggregate the validation project contract.",
    )
    item.add_argument("project", type=Path)
    output_args(item)
    return root
    ...
    try:
        if args.command == "doctor":
            ...
            result = compare_meshes(
                args.reference, args.candidate, policy(args.policy), args.profile
            )
        ...
        elif args.command == "validate-p2-stage":
            result = validate_p2_stage(args.manifest, args.profile)
        elif args.command == "author-anycubic-3mf":
            result = author_anycubic_3mf(
                args.source,
                args.output,
                machine_profile=args.machine_profile,
                process_profile=args.process_profile,
            ...
            result = freeze_project(
                args.project, args.output, force=args.force, profile=args.profile
            )
        ...
        result = report(
            args.command,
            [check("unhandled-error", "FAIL", f"{type(exc).__name__}: {exc}")],
