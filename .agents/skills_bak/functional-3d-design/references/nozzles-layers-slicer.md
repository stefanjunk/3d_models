...
For deliberate time/material optimization, use `optimize-fdm-design`. Prefer a few continuous load-oriented ribs, shells, and local modifiers over global high infill. Slice modeled window/lattice patterns before accepting them: many small cells can create enough perimeter/corner/retraction burden to print slower than a simpler wall.
...
- record model and support material separately, estimated time, triangle/import/slice burden, and warnings for the baseline and selected optimization candidate;
