...
2. Read `references/watermark-release-gate.md` and copy the bundled `JSI-WM-001-R1` assets into the project without redrawing or substituting text.
...
4. Run `scripts/select_watermark.py` with the safe-region size, host-wall thickness, nozzle, and layer height. Use its standard/compact choice, scale, rotation, clearance, and depth unless a documented engineering reason requires a stricter result.
