...
def main() -> int:
    ...
    p.add_argument("--output-root", type=Path, help="Directory for generated reports and example exports")
    args = p.parse_args()
    ...
    generated_root = (args.output_root or package_root / "generated").resolve()
    ...
    for name in selected:
        ...
        if name in {"honeycomb-wall-shelf", "rounded-desk-organizer"}:
            if not have_cq:
                ...
                item["passed"] = item["passed"] and not args.strict_tools
            else:
        ...
        item["outputs"] = [str(path.relative_to(generated_root)) for path in sorted(out.iterdir()) if path.is_file()]
        for stl in stls:
            validation = run([
                sys.executable,
