// Monolithic honeycomb hair clip, revision 5.
...
function assertParams (p) {
  ...
  if (p.armorLongitudinalScale < 0.94 || p.armorLongitudinalScale > 1.0) throw new Error('armorLongitudinalScale outside validated range 0.94-1.0')
...
function regularHexagon (centerA, centerB, circumradius, longitudinalScale = 1) {
  const points = []
  for (let i = 0; i < 6; i++) {
    const angle = i * Math.PI / 3
    points.push([
      centerA + circumradius * longitudinalScale * Math.cos(angle),
...
// Clip a convex X/Z footprint at the build plane. Revision 5 intentionally
...
function makeArmor (p) {
  ...
  // Keep the approved 10 mm key width in Z and the real 0.8 mm groove while
  // compressing X by 4.3%. This lets complete protruding cells form the same
  // stepped outline at both ends without exceeding the approved 65 mm length.
  const radiusX = radius * p.armorLongitudinalScale
  const pitchX = 3 * radiusX + Math.sqrt(3) * p.armorGap
  const pitchZ = (p.armorAcrossFlats + p.armorGap) / 2
  const gridCenterX = (p.shellStartX + p.shellEndX) / 2
  const evenRowStartX = gridCenterX - pitchX
  const oddRowStartX = gridCenterX - 1.5 * pitchX
  const rowCount = 5
  const cellsPerRow = [3, 4, 3, 4, 3]
  ...
  // pointy-ended cells in one uniform orientation. Rows 1 and 3 contain four
  // complete cells, so their outer cells project equally over the hinge and
  // latch ends. This mirrors the requested high/low end contour. Row 4 remains
  // complete and creates the high/low long edge opposite the build plate.
  for (let row = 0; row < rowCount; row++) {
    const centerZ = row * pitchZ
    const firstCenterX = row % 2 === 0 ? evenRowStartX : oddRowStartX
    for (let column = 0; column < cellsPerRow[row]; column++) {
      const centerX = firstCenterX + column * pitchX
      const wholeFootprint = regularHexagon(centerX, centerZ, radius, p.armorLongitudinalScale)
...
function gridEndX (p) {
  ...
  const radiusX = radius * p.armorLongitudinalScale
  const pitchX = 3 * radiusX + Math.sqrt(3) * p.armorGap
  const centerX = (p.shellStartX + p.shellEndX) / 2
  const extent = 1.5 * pitchX + radiusX
  return { min: centerX - extent, max: centerX + extent }
}
...
function writeBinaryStl (filename, mesh) {
  ...
  out.write('Monolithic honeycomb PETG hair clip r5', 0, 'ascii')
...
function main () {
  ...
  const result = {
    ...
    quality,
    armor: {
      cellShape: 'uniform-pointy-near-regular-hexagon',
      ...
      hingeContourMinXmm: gridEndX(p).min,
      latchContourMaxXmm: gridEndX(p).max,
      hingeOverhangPastShellMm: p.shellStartX - gridEndX(p).min,
      latchOverhangPastShellMm: gridEndX(p).max - p.shellEndX,
      mirroredEndContour: true,
    ...
    clip: writeGeometry(clip, 'masculine-honeycomb-hair-clip-r5', outputDir, true),
    coupon: writeGeometry(coupon, 'hair-clip-latch-coupon-r5', outputDir, false)
  }
