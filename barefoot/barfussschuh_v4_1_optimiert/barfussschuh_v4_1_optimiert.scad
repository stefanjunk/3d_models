/*
Parametrischer Barfußschuh V4.1 – optimierte Sohle und Textilschnittmuster
================================
...
/* [Performance und Vorschau] */
// F5 zeigt standardmäßig eine vereinfachte Sohle.
// F6 und Exporte enthalten weiterhin die vollständige Geometrie.
            ...
            for (y = [0 : grip_spacing : sole_length]) {
                local_half_width =
                    max(0,
                        ...
                        edge_allowance -
                        grip_margin);
                local_center = center_shift_at(y);
                row_shift =
                    ((floor(y / grip_spacing) % 2) *
                     grip_spacing / 2);

                for (x = [-local_half_width :
                           grip_spacing :
                           local_half_width])
                    translate([
                        local_center + x + row_shift,
                        y
                    ])
                        circle(
                            d = grip_size,
...
module footprint_pattern_left_2d() {
    union() {
        translate([0, 0.12 * sole_length])
            scale([0.42 * heel_width, 0.10 * sole_length]) circle(r = 1, $fn = footprint_facets);

        translate([-0.05 * waist_width, 0.40 * sole_length])
            rotate(-8)
                scale([0.20 * waist_width, 0.16 * sole_length]) circle(r = 1, $fn = footprint_facets);

        translate([0, 0.69 * sole_length])
            scale([0.50 * ball_width, 0.09 * sole_length]) circle(r = 1, $fn = footprint_facets);

        translate([0.26 * toe_box_width, 0.89 * sole_length])
            scale([0.13 * toe_box_width, 0.06 * sole_length]) circle(r = 1, $fn = footprint_facets);
        ...
        for (i = [0 : 3])
            translate([
                ...
                scale([
                    ...
                    circle(r = 1, $fn = footprint_facets);
    }
}

module outsole_positive_geometry_left() {
...
module outsole_positive_left() {
    if (!$preview || preview_show_grip) {
        if (use_render_barriers && !$preview)
            render(convexity = 4)
                outsole_positive_geometry_left();
        else
            outsole_positive_geometry_left();
    }
}
...
module sole_core_left() {
    difference() {
        ...
        if (internal_structure == "light_cells" &&
            (!$preview || preview_show_internal_cells)) {
            if (use_render_barriers && !$preview)
                render(convexity = 6)
...
module pat_label(txt, x, y, rot = 0) {
    if (pattern_include_labels &&
        (!$preview || preview_show_pattern_labels))
        translate([x, y])
            rotate(rot)
                text(
                    txt,
