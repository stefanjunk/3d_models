...
The generated workbook now presents one filterable `Portfolio` list with all 422 records: 108 current product directories and 314 planned products/research ideas. Its leading canonical columns expose status, commercial data, C/R/K, current and target lanes, dimensions, materials, manufacturing estimates, scores and next gates side by side; source-prefixed columns preserve every joined cell for lossless audit. The prior 99-row curated product list remains unchanged in `Product Register`, and `Product Preflights` retains all 108 exact path-based preflight records. Every one of those 108 products links both a validated `preflight/preflight-result.json` and an explicit `PURPOSE.md`. The compact format is `C# · R# · K# · Lane X · CONFIDENCE`; `Target_Lane_After_Evidence` remains a separate planning field and never overrides the current lane or release decision.
