...
Both research tabs also carry row-level preflight planning fields. The 37 ideas already mapped to a product use its current scorecard; the remaining 163 use clearly labeled preliminary bands with `R0–R1` and current `Lane E`. These estimates support side-by-side market-potential/implementation-complexity analysis only and are not product preflights, safety qualifications, or release approvals. The method and regeneration commands are documented in `implementation-priority-scoring.md`.
