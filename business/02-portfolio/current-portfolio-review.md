...
The generated workbook now presents one filterable `Portfolio` list with all 580 records: 109 current product directories and 471 planned products/research ideas. Every row has one populated, unique `Working_SKU`; the redundant `Record_ID`, `Mapped_Working_SKU`, and duplicate `Product Register` worksheet are omitted. Its leading canonical columns expose status, commercial data, C/R/K, current and target lanes, dimensions, materials, manufacturing estimates, scores and next gates side by side; source-prefixed columns preserve the remaining joined evidence fields for audit. The prior 99-row curated product source remains version-controlled in `product-portfolio.csv`, and all 108 exact path-based product preflights remain in their JSON/PURPOSE sources and joined Portfolio fields. The compact format is `C# · R# · K# · Lane X · CONFIDENCE`; `Target_Lane_After_Evidence` remains a separate planning field and never overrides the current lane or release decision.

The workbook now covers 471 research ideas. The 37 ideas already mapped to a product use its current scorecard; 163 legacy ideas use clearly labeled preliminary bands with `R0–R1` and current `Lane E`. SKU-201–300 add 100 explicit-purpose ideas with directional trend scores of 87–94 and structured `K1 · C1–C2 · R2` concept preflights. They remain `Lane E`, `LOW_UNKNOWN`, and `CONCEPT_ONLY` until exact process and variant evidence close.

SKU-301–314 and SKU-501–557 are separate, named-interface children of retained generic ideas. SKU-501–557 adds 57 children for the ten highest trend-score records (SKU-130, SKU-112, SKU-178, SKU-200, SKU-133, SKU-171, SKU-173, SKU-001, SKU-160, SKU-107), each specified to one named IKEA article, published ISO/ICAO format, mint coin set, USB-IF connector, drawer system or named craft/sewing consumable; All 71 have explicit purpose, primary-source nominal interface evidence, a hashed research-only Kobra 3 Max/PETG process baseline, measurable acceptance criteria, and compact `C1–C3 · R3 · K1 · Lane B/C · CONDITIONAL` preflights. Across both waves eight are C1, 38 C2, and 25 C3. Their generic parents do not inherit R3. R3 here means nominal design inputs are complete enough to start controlled CAD; it does not mean a real device/host has fitted, a coupon has passed, demand is proven, or commercial release is approved.

SKU-315–414 are the 100 generative Step1X-3D research concepts added on 2026-09-04 for the requested popular-model groups: 20 animals and creatures, 18 cartoon and comic characters, 18 toys and fidgets, 16 tools and desktop utility items, 14 persons and figurines, and 14 trending decor objects. Each row is an appearance-led product whose shell is planned as one AI reference image converted by Step1X-3D and finished parametrically in CAD, and each row records the image prompt, the generative route, a mandatory mesh-quality gate, an IP basis, an AI-transparency duty, a directional trend score above 70 with named source IDs, an `R2 · C1–C3 · K1–K2` concept preflight, and a modeled unit-economics block (mass, print time, hands-on minutes, cost components, recommended price, contribution margin) computed on the retained Unit Economics rates. Trend values come from primary platform APIs, company filings and platform trend reports; where only category-level or unverified evidence exists, the row says so instead of quoting a number. `G3` stays failing by design because no exact printer, filament batch, nozzle or process profile is pinned. The `TOOL-LICENCE` gate stands at `WARN`: the blocking conflict was resolved in the owned fork `github.com/stefanjunk/Step1X-3D`, which removed the Tencent Hunyuan-derived code that excluded the European Union — eleven files with the deleted texture stage, the twelfth replaced by an independent volume decoder verified to reproduce the same iso-surface — together with the Stable Diffusion XL RAIL++-M flow-down duty and every copyleft dependency in the inference path. It remains a `WARN` rather than closing because the `openai/clip-vit-large-patch14` weights the visual encoder loads declare no licence, the published weights' Objaverse training-data provenance is unresolved upstream, and the image generator for step one is fixed per SKU. The batch stays `Lane E`, `LOW_UNKNOWN` and `CONCEPT_ONLY` on the process, rights and demand gates regardless. The component-level licence audit behind that gate — every code, weight and dependency layer that runs, read from the local checkout, the running container and the populated model cache — is in `business/06-legal-compliance/generative-tooling-licence-audit.md`, together with the ordered list of checks that must close before EU commercial use. The modeled prices and margins order planning work only; they are not quotations, measured prints, proven demand or approved margins.

`readiness-advancement-register.csv` and the workbook tab `R Advancement` assess every one of the 471 ideas and 109 product directories. Each row records purpose, current compact preflight, priority wave, advancement potential, evidence boundary, bottleneck, and exact next evidence. This makes the requested high-trend/low-complexity-first sequence explicit without averaging market potential into C/R/K.
