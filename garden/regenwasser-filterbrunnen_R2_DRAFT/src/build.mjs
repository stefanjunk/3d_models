...
const outputRoot = path.join(projectRoot, "build", "draft-r2");
...
const partDefinitions = [
  ...
  {
    key: "hose_barb_flange_25",
    name: "25-mm-Schlauchtuelle mit Flansch",
    quantity: 2,
    builder: buildHoseBarbFlange,
    orientation: "as-designed",
  },
  {
...
function buildAssembly(partShapes) {
  ...
  addAssemblyShape(
    assembly,
    partShapes.stage2_diffuser.clone().translateZ(d.stage2AssemblyZ + 14),
    "Diffusor",
    "#0284c7"
  );
  ...
  addAssemblyShape(
    assembly,
    partShapes.hose_barb_flange_25.clone()
...
async function run() {
  ...
  for (const definition of partDefinitions) {
    ...
    if (!checkOnly) {
      const stem = `DRAFT_R2_${definition.key}`;
  ...
  if (!checkOnly) {
    const assemblyStepPath = path.join(stepDir, "DRAFT_R2_filterbrunnen_assembly.step");
    await writeBlob(
      assemblyStepPath,
    ...
    const previewStlPath = path.join(previewDir, "DRAFT_R2_filterbrunnen_assembly_preview.stl");
    await writeBlob(
      previewStlPath,
      compound.blobSTL({
        tolerance: 0.35,
