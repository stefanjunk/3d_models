...
FUNNEL_DEPTH = 2.5
FUNNEL_ENTRY_R = 23.0      # larger visible upper funnel radius for more catch/drain area
FUNNEL_FLOOR_R = 19.0      # larger lower capture floor radius; outer ring stays hole-free
FUNNEL_FLOOR_Z = TOTAL_HEIGHT - FUNNEL_DEPTH
HOLE_D = 2.8               # slightly larger for more throughflow while still retaining hair reasonably well
HOLE_PITCH = 4.3
HOLE_FIELD_R = 16.0        # larger sieve field, still leaves a solid rim between sieve and funnel wall
STEP_X = SEGMENT_LENGTH / 4.0
CATCHER_XS = [STEP_X * 0.5, STEP_X * 1.5, STEP_X * 2.5, STEP_X * 3.5]  # four larger catchers per segment
CATCHER_YS = [TOTAL_WIDTH / 2.0]

# Swirl hair-guiding ribs — start farther out, curl inward
RIB_W = 1.6
RIB_H = FUNNEL_DEPTH - 0.02  # 0.02 mm CAD relief; first layer still lands on bed when printed upside down
RIB_COUNT = 5
RIB_START_R = FUNNEL_ENTRY_R - RIB_W/2.0 + 0.05
RIB_END_R = 10.2
RIB_SWEEP = math.radians(126)
RIB_STEPS = 24
CENTER_BOSS_R = 2.8
...
def make_rib_solid(cx, cy, phase_rad):
    # Create a vertical rib prism, then intersect it with the funnel volume.
    # This makes the rib start right at the outer funnel edge and follow the funnel slope inward,
    # so the outer rib end is flush with the surrounding top surface with no radial gap.
    pts = spiral_band_points(phase_rad)
    prism = (cq.Workplane("XY", origin=(cx, cy, FUNNEL_FLOOR_Z))
             .polyline(pts).close().extrude(FUNNEL_DEPTH + 0.04))
    funnel_vol = make_funnel_tool(cx, cy)
    return prism.intersect(funnel_vol)
...
def add_catchers(panel, centers):
    ...
    for cx, cy in centers:
        for k in range(RIB_COUNT):
            rib = make_rib_solid(cx, cy, k * 2.0 * math.pi / RIB_COUNT)
            panel = panel.union(rib, clean=False)
        boss = (cq.Workplane("XY", origin=(cx, cy, FUNNEL_FLOOR_Z))
                .circle(CENTER_BOSS_R).extrude(max(0.8, FUNNEL_DEPTH * 0.52)))
...
def write_readme():
    ...
    txt = f"""# Shower drain hair trap – exact 945 × 65 × 20 mm, inverted U-profile, funnel catcher variant – edge-start swirl-rib revision
...
def main():
    ...
    export_shape(tile, "functional_test_tile_80mm")
