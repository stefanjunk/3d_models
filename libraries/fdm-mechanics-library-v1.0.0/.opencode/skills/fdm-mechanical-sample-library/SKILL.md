...
description: Select, compare, adapt, rebuild, and validate a local library of 156 parametric FDM mechanical samples covering hinges, multi-axis joints, permanent and reusable connections, screws, rails, gears, couplers, sealing interfaces, cable glands, oscillators, and component fixtures. Use when a product needs a printable mechanical connection or motion primitive, when FDM clearance variants must be compared, or when a proven sample should be integrated into a larger CAD or OpenSCAD project.
...
- unlike micro-shaft joining → compact asymmetric coupler;
- rotation-to-oscillation → crank-pin and slotted-rocker oscillator;
- sealed rotating shaft → radial O-ring shaft gland;
- sealed reusable closure → O-ring-preloaded ramped bayonet;
- sealed linear displacement → dual O-ring friction piston;
- serviceable pinned joint → captive hinge pin;
- cable through enclosure wall → compression cable gland;
- cylindrical battery retention → cell cradle with purchased contacts;
- non-penetrating magnetic actuation → sealed actuator pocket;
