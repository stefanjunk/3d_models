...
// Use a grayscale PNG/JPG heightmap-like image. With invert=true, dark areas
// engrave deeper and white areas engrave shallower (OpenSCAD surface convention).
// when IMAGE_TEXTURE_INVERT = true. For a cleaner result on flat panels you
// will often want TEXTURE = false when using this image engraving.
IMAGE_TEXTURE_ENABLE = true;
IMAGE_TEXTURE_FILE = "carbon_fiber_heightmap_drawer_213x67.png";   // 0.4 mm physical sampling
...
module drawer_housing() {
    ...
    apply_image_engraving(0,0,0,w,d,h,front=false,sides=false) {
        union() {
...
module drawer() {
    ...
    union() {
        ...
        // Keep the pull outside the heightmap Boolean so it cannot fragment.
...
module shallow_tray() {
    ...
    key_x1 = 18; key_x2 = w-18-key_w; key_y = (d-key_d)/2;
    apply_image_engraving(0,0,0,w,d,h,front=true,sides=false) {
        difference() {
            union() {
                cup(w,d,h,3,3,r);
