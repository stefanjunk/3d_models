...
* @experimental
*/
var FeatureID = class extends _gltf_transform_core.ExtensionProperty {
...
* @experimental
*/
var FeatureIDTexture = class extends _gltf_transform_core.ExtensionProperty {
	static EXTENSION_NAME = EXT_MESH_FEATURES;
	init() {
		this.extensionName = EXT_MESH_FEATURES;
	...
	getDefaults() {
		const defaultTextureInfo = new _gltf_transform_core.TextureInfo(this.graph, "textureInfo");
		defaultTextureInfo.setMinFilter(_gltf_transform_core.TextureInfo.MagFilter.NEAREST);
		defaultTextureInfo.setMagFilter(_gltf_transform_core.TextureInfo.MagFilter.NEAREST);
		return Object.assign(super.getDefaults(), {
			channels: [0],
			texture: null,
...
* @experimental
*/
var Features = class extends _gltf_transform_core.ExtensionProperty {
	static EXTENSION_NAME = EXT_MESH_FEATURES;
	init() {
		this.extensionName = EXT_MESH_FEATURES;
		this.propertyType = "Features";
		this.parentTypes = [_gltf_transform_core.PropertyType.PRIMITIVE];
	}
	getDefaults() {
		return Object.assign(super.getDefaults(), { featureIds: new _gltf_transform_core.RefSet([]) });
	}
	listFeatureIDs() {
...
* @experimental
*/
var EXTMeshFeatures = class extends _gltf_transform_core.Extension {
...
function _writeFeatureIDDef(document, context, featureID) {
	...
	if (featureID.getTexture()) {
		...
		if (!_gltf_transform_core.MathUtils.eq(channels, [0])) featureIDDef.texture.channels = channels;
	}
...
*/
var InstancedMesh = class extends _gltf_transform_core.ExtensionProperty {
	static EXTENSION_NAME = EXT_MESH_GPU_INSTANCING;
	init() {
		this.extensionName = EXT_MESH_GPU_INSTANCING;
		this.propertyType = "InstancedMesh";
		this.parentTypes = [_gltf_transform_core.PropertyType.NODE];
	}
	getDefaults() {
		return Object.assign(super.getDefaults(), { attributes: new _gltf_transform_core.RefMap() });
	}
...
*/
var EXTMeshGPUInstancing = class extends _gltf_transform_core.Extension {
	static EXTENSION_NAME = EXT_MESH_GPU_INSTANCING;
	extensionName = EXT_MESH_GPU_INSTANCING;
	/** @hidden */
	prewriteTypes = [_gltf_transform_core.PropertyType.ACCESSOR];
...
const { BYTE, SHORT, FLOAT } = _gltf_transform_core.Accessor.ComponentType;
const { encodeNormalizedInt, decodeNormalizedInt } = _gltf_transform_core.MathUtils;
...
function padArrayElements(srcArray, elementSize) {
	const elementStride = _gltf_transform_core.BufferUtils.padNumber(srcArray.BYTES_PER_ELEMENT * elementSize) / srcArray.BYTES_PER_ELEMENT;
...
function getMeshoptMode(accessor, usage) {
	if (usage === _gltf_transform_core.WriterContext.BufferViewUsage.ELEMENT_ARRAY_BUFFER) return accessor.listParents().some((parent) => {
		return parent instanceof _gltf_transform_core.Primitive && parent.getMode() === _gltf_transform_core.Primitive.Mode.TRIANGLES;
	}) ? "TRIANGLES" : "INDICES";
	return "ATTRIBUTES";
}
function getMeshoptFilter(accessor, doc) {
	const refs = doc.getGraph().listParentEdges(accessor).filter((edge) => !(edge.getParent() instanceof _gltf_transform_core.Root));
	for (const ref of refs) {
		const refName = ref.getName();
		const refKey = ref.getAttributes().key || "";
		const isDelta = ref.getParent().propertyType === _gltf_transform_core.PropertyType.PRIMITIVE_TARGET;
...
function getTargetPath(accessor) {
	for (const sampler of accessor.listParents()) {
		if (!(sampler instanceof _gltf_transform_core.AnimationSampler)) continue;
		for (const channel of sampler.listParents()) {
			if (!(channel instanceof _gltf_transform_core.AnimationChannel)) continue;
...
* class.
*/
var EXTMeshoptCompression = class extends _gltf_transform_core.Extension {
	extensionName = EXT_MESHOPT_COMPRESSION;
	/** @hidden */
	prereadTypes = [_gltf_transform_core.PropertyType.BUFFER, _gltf_transform_core.PropertyType.PRIMITIVE];
	/** @hidden */
	prewriteTypes = [_gltf_transform_core.PropertyType.BUFFER, _gltf_transform_core.PropertyType.ACCESSOR];
	/** @hidden */
	...
	*/
	...
	preread(context, propertyType) {
		...
		if (propertyType === _gltf_transform_core.PropertyType.BUFFER) this._prereadBuffers(context);
		else if (propertyType === _gltf_transform_core.PropertyType.PRIMITIVE) this._prereadPrimitives(context);
		return this;
	}
	...
	_prereadBuffers(context) {
		...
		(jsonDoc.json.bufferViews || []).forEach((viewDef, index) => {
			...
			const resource = bufferDef.uri ? jsonDoc.resources[bufferDef.uri] : jsonDoc.resources[_gltf_transform_core.GLB_BUFFER];
			const source = _gltf_transform_core.BufferUtils.toView(resource, byteOffset, byteLength);
	...
	*/
	...
	read(_context) {
		...
		for (const [fallbackBuffer, buffer] of this._decoderFallbackBufferMap) {
			for (const parent of fallbackBuffer.listParents()) if (parent instanceof _gltf_transform_core.Accessor) parent.swap(fallbackBuffer, buffer);
	...
	*/
	...
	prewrite(context, propertyType) {
		if (propertyType === _gltf_transform_core.PropertyType.ACCESSOR) this._prewriteAccessors(context);
		else if (propertyType === _gltf_transform_core.PropertyType.BUFFER) this._prewriteBuffers(context);
		return this;
	}
	...
	_prewriteAccessors(context) {
		...
		const getParentID = (property) => {
			for (const parent of graph.listParents(property)) {
				if (parent.propertyType === _gltf_transform_core.PropertyType.ROOT) continue;
		...
		for (const accessor of this.document.getRoot().listAccessors()) {
			...
			if (!bufferView || !bufferViewData) {
				...
				bufferView = this._encoderBufferViews[key] = {
					...
					target: _gltf_transform_core.WriterContext.USAGE_TO_TARGET[usage],
					byteOffset: 0,
					byteLength: 0,
					byteStride: usage === _gltf_transform_core.WriterContext.BufferViewUsage.ARRAY_BUFFER ? byteStride : void 0,
	...
	_prewriteBuffers(context) {
		...
		for (const key in this._encoderBufferViews) {
			...
			const srcArray = _gltf_transform_core.BufferUtils.concat(bufferViewData);
			const dstArray = encoder.encodeGltfBuffer(srcArray, count, byteStride, mode);
			const compressedData = _gltf_transform_core.BufferUtils.pad(dstArray);
	...
	write(context) {
		...
		for (const key in this._encoderBufferViews) {
			...
			fallbackBufferByteOffset += _gltf_transform_core.BufferUtils.padNumber(bufferView.byteLength);
		}
...
* @experimental
*/
var StructuralMetadata = class extends _gltf_transform_core.ExtensionProperty {
	...
	init() {
		...
		this.parentTypes = [_gltf_transform_core.PropertyType.ROOT];
	}
	getDefaults() {
		return Object.assign(super.getDefaults(), {
			schema: null,
			schemaUri: "",
			propertyTables: new _gltf_transform_core.RefList(),
			propertyTextures: new _gltf_transform_core.RefList(),
			propertyAttributes: new _gltf_transform_core.RefList()
		});
	}
	getSchema() {
...
* @experimental
*/
var Schema = class extends _gltf_transform_core.ExtensionProperty {
	static EXTENSION_NAME = EXT_STRUCTURAL_METADATA;
	init() {
		this.extensionName = EXT_STRUCTURAL_METADATA;
	...
	getDefaults() {
		return Object.assign(super.getDefaults(), {
			description: "",
			version: "",
			classes: new _gltf_transform_core.RefMap(),
			enums: new _gltf_transform_core.RefMap()
		});
	}
	getId() {
		return this.get("id");
...
* @experimental
*/
var Class = class extends _gltf_transform_core.ExtensionProperty {
	static EXTENSION_NAME = EXT_STRUCTURAL_METADATA;
	init() {
		this.extensionName = EXT_STRUCTURAL_METADATA;
	...
	getDefaults() {
		return Object.assign(super.getDefaults(), {
			description: "",
			properties: new _gltf_transform_core.RefMap()
		});
	}
	getDescription() {
		return this.get("description");
...
* @experimental
*/
var ClassProperty = class extends _gltf_transform_core.ExtensionProperty {
	static EXTENSION_NAME = EXT_STRUCTURAL_METADATA;
	init() {
		this.extensionName = EXT_STRUCTURAL_METADATA;
...
* @experimental
*/
var Enum = class extends _gltf_transform_core.ExtensionProperty {
	static EXTENSION_NAME = EXT_STRUCTURAL_METADATA;
	init() {
		this.extensionName = EXT_STRUCTURAL_METADATA;
	...
	getDefaults() {
		return Object.assign(super.getDefaults(), {
			description: "",
			valueType: "UINT16",
			values: new _gltf_transform_core.RefList()
		});
	}
	getDescription() {
		return this.get("description");
...
* @experimental
*/
var EnumValue = class extends _gltf_transform_core.ExtensionProperty {
	static EXTENSION_NAME = EXT_STRUCTURAL_METADATA;
	init() {
		this.extensionName = EXT_STRUCTURAL_METADATA;
...
* @experimental
*/
var PropertyTable = class extends _gltf_transform_core.ExtensionProperty {
	static EXTENSION_NAME = EXT_STRUCTURAL_METADATA;
	init() {
		this.extensionName = EXT_STRUCTURAL_METADATA;
	...
	getDefaults() {
		return Object.assign(super.getDefaults(), { properties: new _gltf_transform_core.RefMap() });
	}
	getClass() {
...
* @experimental
*/
var PropertyTableProperty = class extends _gltf_transform_core.ExtensionProperty {
	static EXTENSION_NAME = EXT_STRUCTURAL_METADATA;
	init() {
		this.extensionName = EXT_STRUCTURAL_METADATA;
...
* @experimental
*/
var PropertyTexture = class extends _gltf_transform_core.ExtensionProperty {
	static EXTENSION_NAME = EXT_STRUCTURAL_METADATA;
	init() {
		this.extensionName = EXT_STRUCTURAL_METADATA;
...
* @experimental
*/
var PropertyTextureProperty = class extends _gltf_transform_core.ExtensionProperty {
	static EXTENSION_NAME = EXT_STRUCTURAL_METADATA;
	init() {
		this.extensionName = EXT_STRUCTURAL_METADATA;
...
* @experimental
*/
var PropertyAttribute = class extends _gltf_transform_core.ExtensionProperty {
	static EXTENSION_NAME = EXT_STRUCTURAL_METADATA;
	init() {
		this.extensionName = EXT_STRUCTURAL_METADATA;
...
* @experimental
*/
var PropertyAttributeProperty = class extends _gltf_transform_core.ExtensionProperty {
	static EXTENSION_NAME = EXT_STRUCTURAL_METADATA;
	init() {
		this.extensionName = EXT_STRUCTURAL_METADATA;
...
* @experimental
*/
var NodeStructuralMetadata = class extends _gltf_transform_core.ExtensionProperty {
	static EXTENSION_NAME = EXT_STRUCTURAL_METADATA;
	init() {
		this.extensionName = EXT_STRUCTURAL_METADATA;
...
* @experimental
*/
var MeshPrimitiveStructuralMetadata = class extends _gltf_transform_core.ExtensionProperty {
	static EXTENSION_NAME = EXT_STRUCTURAL_METADATA;
	init() {
		this.extensionName = EXT_STRUCTURAL_METADATA;
...
* @experimental
*/
var EXTStructuralMetadata = class extends _gltf_transform_core.Extension {
	...
	* @hidden
	*/
	prewriteTypes = [_gltf_transform_core.PropertyType.BUFFER];
	/**
	* Must read EXT_structural_metadata before EXT_mesh_features.
	*
	* @hidden
	*/
	prereadTypes = [_gltf_transform_core.PropertyType.SCENE];
	...
	prewrite(context, propertyType) {
		if (propertyType === _gltf_transform_core.PropertyType.BUFFER) this._prewriteBuffers(context);
		return this;
	}
	/**
...
function _writePropertyTexturePropertyDef(context, propertyTextureProperty) {
	...
	if (!_gltf_transform_core.MathUtils.eq(channels, [0])) textureInfoDef.channels = channels;
...
function getBufferViewData(context, bufferViewIndex) {
	...
	const bufferData = bufferDef.uri ? jsonDoc.resources[bufferDef.uri] : jsonDoc.resources[_gltf_transform_core.GLB_BUFFER];
...
var AVIFImageUtils = class {
	match(array) {
		return array.length >= 12 && _gltf_transform_core.BufferUtils.decodeText(array.slice(4, 12)) === "ftypavif";
	}
	/**
...
* JPEG image data.
*/
var EXTTextureAVIF = class extends _gltf_transform_core.Extension {
	extensionName = EXT_TEXTURE_AVIF;
	/** @hidden */
	prereadTypes = [_gltf_transform_core.PropertyType.TEXTURE];
	static EXTENSION_NAME = EXT_TEXTURE_AVIF;
	/** @hidden */
	static register() {
		_gltf_transform_core.ImageUtils.registerFormat("image/avif", new AVIFImageUtils());
	}
	/** @hidden */
	preread(context) {
...
function unbox(data, offset) {
	...
	return {
		type: _gltf_transform_core.BufferUtils.decodeText(new Uint8Array(data.buffer, data.byteOffset + offset + 4, 4)),
		start: offset + 8,
		end: offset + size
	};
}
...
var WEBPImageUtils = class {
	...
	getSize(array) {
		const RIFF = _gltf_transform_core.BufferUtils.decodeText(array.slice(0, 4));
		const WEBP = _gltf_transform_core.BufferUtils.decodeText(array.slice(8, 12));
		if (RIFF !== "RIFF" || WEBP !== "WEBP") return null;
		const view = new DataView(array.buffer, array.byteOffset);
		let offset = 12;
		while (offset < view.byteLength) {
			const chunkId = _gltf_transform_core.BufferUtils.decodeText(new Uint8Array([
...
* JPEG image data.
*/
var EXTTextureWebP = class extends _gltf_transform_core.Extension {
	...
	/** @hidden */
	static register() {
		_gltf_transform_core.ImageUtils.registerFormat("image/webp", new WEBPImageUtils());
	}
	/** @hidden */
	preread(context) {
		(context.jsonDoc.json.textures || []).forEach((textureDef) => {
...
*/
var KHRAccessorFloat16 = class extends _gltf_transform_core.Extension {
...
*/
var KHRAccessorFloat64 = class extends _gltf_transform_core.Extension {
	extensionName = NAME;
...
function initDecoderModule(_decoderModule) {
	...
	COMPONENT_ARRAY = {
		[_gltf_transform_core.Accessor.ComponentType.FLOAT]: Float32Array,
		[_gltf_transform_core.Accessor.ComponentType.UNSIGNED_INT]: Uint32Array,
		[_gltf_transform_core.Accessor.ComponentType.UNSIGNED_SHORT]: Uint16Array,
		[_gltf_transform_core.Accessor.ComponentType.UNSIGNED_BYTE]: Uint8Array,
		[_gltf_transform_core.Accessor.ComponentType.SHORT]: Int16Array,
		[_gltf_transform_core.Accessor.ComponentType.BYTE]: Int8Array
	};
	DATA_TYPE = {
		[_gltf_transform_core.Accessor.ComponentType.FLOAT]: decoderModule.DT_FLOAT32,
		[_gltf_transform_core.Accessor.ComponentType.UNSIGNED_INT]: decoderModule.DT_UINT32,
		[_gltf_transform_core.Accessor.ComponentType.UNSIGNED_SHORT]: decoderModule.DT_UINT16,
		[_gltf_transform_core.Accessor.ComponentType.UNSIGNED_BYTE]: decoderModule.DT_UINT8,
		[_gltf_transform_core.Accessor.ComponentType.SHORT]: decoderModule.DT_INT16,
		[_gltf_transform_core.Accessor.ComponentType.BYTE]: decoderModule.DT_INT8
	};
}
//#endregion
...
function addAttribute(builder, componentType, mesh, attribute, count, itemSize, array) {
	switch (componentType) {
		case _gltf_transform_core.Accessor.ComponentType.UNSIGNED_BYTE: return builder.AddUInt8Attribute(mesh, attribute, count, itemSize, array);
		case _gltf_transform_core.Accessor.ComponentType.BYTE: return builder.AddInt8Attribute(mesh, attribute, count, itemSize, array);
		case _gltf_transform_core.Accessor.ComponentType.UNSIGNED_SHORT: return builder.AddUInt16Attribute(mesh, attribute, count, itemSize, array);
		case _gltf_transform_core.Accessor.ComponentType.SHORT: return builder.AddInt16Attribute(mesh, attribute, count, itemSize, array);
		case _gltf_transform_core.Accessor.ComponentType.UNSIGNED_INT: return builder.AddUInt32Attribute(mesh, attribute, count, itemSize, array);
		case _gltf_transform_core.Accessor.ComponentType.FLOAT: return builder.AddFloatAttribute(mesh, attribute, count, itemSize, array);
...
* I/O class.
*/
var KHRDracoMeshCompression = class extends _gltf_transform_core.Extension {
	extensionName = KHR_DRACO_MESH_COMPRESSION;
	/** @hidden */
	prereadTypes = [_gltf_transform_core.PropertyType.PRIMITIVE];
	/** @hidden */
	prewriteTypes = [_gltf_transform_core.PropertyType.ACCESSOR];
	/** @hidden */
	...
	*/
		...
		try {
			...
			for (const meshDef of meshDefs) for (const primDef of meshDef.primitives) {
				...
				if (!dracoMesh || !decoder) {
					...
					const byteOffset = bufferViewDef.byteOffset || 0;
					const byteLength = bufferViewDef.byteLength;
					const compressedData = _gltf_transform_core.BufferUtils.toView(resource, byteOffset, byteLength);
	...
	prewrite(context, _propertyType) {
		...
		else quantizationVolume = (0, _gltf_transform_core.getBounds)(this.document.getRoot().listScenes().pop());
		for (const prim of Array.from(primitiveHashMap.keys())) {
			...
			if (encodedPrim.numVertices > 65534 && _gltf_transform_core.Accessor.getComponentSize(indicesDef.componentType) <= 2) indicesDef.componentType = _gltf_transform_core.Accessor.ComponentType.UNSIGNED_INT;
			else if (encodedPrim.numVertices > 254 && _gltf_transform_core.Accessor.getComponentSize(indicesDef.componentType) <= 1) indicesDef.componentType = _gltf_transform_core.Accessor.ComponentType.UNSIGNED_SHORT;
			for (const semantic of prim.listSemantics()) {
				const attribute = prim.getAttribute(semantic);
...
function listDracoPrimitives(doc) {
	...
	for (const mesh of doc.getRoot().listMeshes()) for (const prim of mesh.listPrimitives()) if (!prim.getIndices()) {
		excluded.add(prim);
		nonIndexed++;
	} else if (prim.getMode() !== _gltf_transform_core.Primitive.Mode.TRIANGLES) {
		excluded.add(prim);
		nonTriangles++;
	...
	for (const accessor of Array.from(includedAccessors.keys())) {
		...
		if (parentTypes.size !== 2 || !parentTypes.has(_gltf_transform_core.PropertyType.PRIMITIVE) || !parentTypes.has(_gltf_transform_core.PropertyType.ROOT)) throw new Error(`[${KHR_DRACO_MESH_COMPRESSION}] Compressed accessors must only be used as indices or vertex attributes.`);
	}
	for (const prim of Array.from(included)) {
...
*/
var Light = class Light extends _gltf_transform_core.ExtensionProperty {
...
* ```
*/
var KHRLightsPunctual = class extends _gltf_transform_core.Extension {
	...
	write(context) {
		...
		for (const property of this.properties) {
			...
			if (!_gltf_transform_core.MathUtils.eq(light.getColor(), [
				1,
				1,
				1
...
var Anisotropy = class extends _gltf_transform_core.ExtensionProperty {
	...
	getDefaults() {
		return Object.assign(super.getDefaults(), {
			...
			anisotropyTextureInfo: new _gltf_transform_core.TextureInfo(this.graph, "anisotropyTextureInfo")
		});
	}
	/**********************************************************************************************
	* Anisotropy strength.
...
* ```
*/
var KHRMaterialsAnisotropy = class extends _gltf_transform_core.Extension {
	static EXTENSION_NAME = KHR_MATERIALS_ANISOTROPY;
	extensionName = KHR_MATERIALS_ANISOTROPY;
	prereadTypes = [_gltf_transform_core.PropertyType.MESH];
	prewriteTypes = [_gltf_transform_core.PropertyType.MESH];
...
const { R: R$6, G: G$6, B: B$4 } = _gltf_transform_core.TextureChannel;
/**
* Defines clear coat for a PBR material. See {@link KHRMaterialsClearcoat}.
*/
var Clearcoat = class extends _gltf_transform_core.ExtensionProperty {
	...
	getDefaults() {
		return Object.assign(super.getDefaults(), {
			...
			clearcoatTextureInfo: new _gltf_transform_core.TextureInfo(this.graph, "clearcoatTextureInfo"),
			clearcoatRoughnessFactor: 0,
			clearcoatRoughnessTexture: null,
			clearcoatRoughnessTextureInfo: new _gltf_transform_core.TextureInfo(this.graph, "clearcoatRoughnessTextureInfo"),
			clearcoatNormalScale: 1,
			clearcoatNormalTexture: null,
			clearcoatNormalTextureInfo: new _gltf_transform_core.TextureInfo(this.graph, "clearcoatNormalTextureInfo")
		});
	}
	/**********************************************************************************************
	* Clearcoat.
...
* ```
*/
var KHRMaterialsClearcoat = class extends _gltf_transform_core.Extension {
	static EXTENSION_NAME = KHR_MATERIALS_CLEARCOAT;
...
const { R: R$5, G: G$5, B: B$3, A: A$3 } = _gltf_transform_core.TextureChannel;
/**
...
*/
var DiffuseTransmission = class extends _gltf_transform_core.ExtensionProperty {
	...
	getDefaults() {
		return Object.assign(super.getDefaults(), {
			...
			diffuseTransmissionTextureInfo: new _gltf_transform_core.TextureInfo(this.graph, "diffuseTransmissionTextureInfo"),
			...
			diffuseTransmissionColorTextureInfo: new _gltf_transform_core.TextureInfo(this.graph, "diffuseTransmissionColorTextureInfo")
		});
	}
	/**********************************************************************************************
...
* @experimental KHR_materials_diffuse_transmission is not yet ratified by the Khronos Group.
*/
var KHRMaterialsDiffuseTransmission = class extends _gltf_transform_core.Extension {
...
*/
var Dispersion = class extends _gltf_transform_core.ExtensionProperty {
...
* ```
*/
var KHRMaterialsDispersion = class extends _gltf_transform_core.Extension {
	static EXTENSION_NAME = KHR_MATERIALS_DISPERSION;
...
*/
var EmissiveStrength = class extends _gltf_transform_core.ExtensionProperty {
...
* ```
*/
var KHRMaterialsEmissiveStrength = class extends _gltf_transform_core.Extension {
	static EXTENSION_NAME = KHR_MATERIALS_EMISSIVE_STRENGTH;
...
*/
var IOR = class extends _gltf_transform_core.ExtensionProperty {
...
* ```
*/
var KHRMaterialsIOR = class extends _gltf_transform_core.Extension {
	static EXTENSION_NAME = KHR_MATERIALS_IOR;
...
const { R: R$4, G: G$4 } = _gltf_transform_core.TextureChannel;
/**
* Defines iridescence (thin film interference) on a PBR {@link Material}. See {@link KHRMaterialsIridescence}.
*/
var Iridescence = class extends _gltf_transform_core.ExtensionProperty {
	...
	getDefaults() {
		return Object.assign(super.getDefaults(), {
			...
			iridescenceTextureInfo: new _gltf_transform_core.TextureInfo(this.graph, "iridescenceTextureInfo"),
			iridescenceIOR: 1.3,
			iridescenceThicknessMinimum: 100,
			iridescenceThicknessMaximum: 400,
			iridescenceThicknessTexture: null,
			iridescenceThicknessTextureInfo: new _gltf_transform_core.TextureInfo(this.graph, "iridescenceThicknessTextureInfo")
		});
	}
	/**********************************************************************************************
	* Iridescence.
...
* ```
*/
var KHRMaterialsIridescence = class extends _gltf_transform_core.Extension {
	static EXTENSION_NAME = KHR_MATERIALS_IRIDESCENCE;
...
const { R: R$3, G: G$3, B: B$2, A: A$2 } = _gltf_transform_core.TextureChannel;
/**
* Converts a {@link Material} to a spec/gloss workflow. See {@link KHRMaterialsPBRSpecularGlossiness}.
*/
var PBRSpecularGlossiness = class extends _gltf_transform_core.ExtensionProperty {
	...
	getDefaults() {
		return Object.assign(super.getDefaults(), {
			...
			diffuseTexture: null,
			diffuseTextureInfo: new _gltf_transform_core.TextureInfo(this.graph, "diffuseTextureInfo"),
			specularFactor: [
				1,
				1,
				1
			...
			specularGlossinessTextureInfo: new _gltf_transform_core.TextureInfo(this.graph, "specularGlossinessTextureInfo")
		});
	}
	/**********************************************************************************************
	* Diffuse.
...
* ```
*/
var KHRMaterialsPBRSpecularGlossiness = class extends _gltf_transform_core.Extension {
	static EXTENSION_NAME = KHR_MATERIALS_PBR_SPECULAR_GLOSSINESS;
...
const { R: R$2, G: G$2, B: B$1, A: A$1 } = _gltf_transform_core.TextureChannel;
/**
* Defines sheen on a PBR {@link Material}. See {@link KHRMaterialsSheen}.
*/
var Sheen = class extends _gltf_transform_core.ExtensionProperty {
	...
	getDefaults() {
		return Object.assign(super.getDefaults(), {
			...
			sheenColorTextureInfo: new _gltf_transform_core.TextureInfo(this.graph, "sheenColorTextureInfo"),
			sheenRoughnessFactor: 0,
			sheenRoughnessTexture: null,
			sheenRoughnessTextureInfo: new _gltf_transform_core.TextureInfo(this.graph, "sheenRoughnessTextureInfo")
		});
	}
	/**********************************************************************************************
	* Sheen color.
...
* ```
*/
var KHRMaterialsSheen = class extends _gltf_transform_core.Extension {
	static EXTENSION_NAME = KHR_MATERIALS_SHEEN;
...
const { R: R$1, G: G$1, B, A } = _gltf_transform_core.TextureChannel;
/**
* Defines specular reflectivity on a PBR {@link Material}. See {@link KHRMaterialsSpecular}.
*/
var Specular = class extends _gltf_transform_core.ExtensionProperty {
	...
	getDefaults() {
		return Object.assign(super.getDefaults(), {
			...
			specularTextureInfo: new _gltf_transform_core.TextureInfo(this.graph, "specularTextureInfo"),
			specularColorFactor: [
				1,
				1,
				1
			...
			specularColorTextureInfo: new _gltf_transform_core.TextureInfo(this.graph, "specularColorTextureInfo")
		});
	}
	/**********************************************************************************************
	* Specular.
...
* ```
*/
var KHRMaterialsSpecular = class extends _gltf_transform_core.Extension {
	static EXTENSION_NAME = KHR_MATERIALS_SPECULAR;
	...
	prewrite(context) {
		...
		this.document.getRoot().listMaterials().forEach((material) => {
			...
			if (specular) {
				...
				if (!_gltf_transform_core.MathUtils.eq(specular.getSpecularColorFactor(), [
					1,
					1,
					1
...
const { R } = _gltf_transform_core.TextureChannel;
/**
* Defines optical transmission on a PBR {@link Material}. See {@link KHRMaterialsTransmission}.
*/
var Transmission = class extends _gltf_transform_core.ExtensionProperty {
	...
	getDefaults() {
		return Object.assign(super.getDefaults(), {
			...
			transmissionTextureInfo: new _gltf_transform_core.TextureInfo(this.graph, "transmissionTextureInfo")
		});
	}
	/**********************************************************************************************
	* Transmission.
...
* ```
*/
var KHRMaterialsTransmission = class extends _gltf_transform_core.Extension {
	static EXTENSION_NAME = KHR_MATERIALS_TRANSMISSION;
...
*/
var Unlit = class extends _gltf_transform_core.ExtensionProperty {
...
* ```
*/
var KHRMaterialsUnlit = class extends _gltf_transform_core.Extension {
	static EXTENSION_NAME = KHR_MATERIALS_UNLIT;
...
*/
var Mapping = class extends _gltf_transform_core.ExtensionProperty {
	...
	getDefaults() {
		return Object.assign(super.getDefaults(), {
			material: null,
			variants: new _gltf_transform_core.RefSet()
		});
	}
...
*/
var MappingList = class extends _gltf_transform_core.ExtensionProperty {
	static EXTENSION_NAME = KHR_MATERIALS_VARIANTS;
	init() {
		this.extensionName = KHR_MATERIALS_VARIANTS;
	...
	getDefaults() {
		return Object.assign(super.getDefaults(), { mappings: new _gltf_transform_core.RefSet() });
	}
...
*/
var Variant = class extends _gltf_transform_core.ExtensionProperty {
	static EXTENSION_NAME = KHR_MATERIALS_VARIANTS;
	init() {
		this.extensionName = KHR_MATERIALS_VARIANTS;
...
*/
var KHRMaterialsVariants = class extends _gltf_transform_core.Extension {
...
const { G } = _gltf_transform_core.TextureChannel;
/**
* Defines volume on a PBR {@link Material}. See {@link KHRMaterialsVolume}.
*/
var Volume = class extends _gltf_transform_core.ExtensionProperty {
	...
	getDefaults() {
		return Object.assign(super.getDefaults(), {
			...
			thicknessTextureInfo: new _gltf_transform_core.TextureInfo(this.graph, "thicknessTexture"),
...
*/
var KHRMaterialsVolume = class extends _gltf_transform_core.Extension {
	static EXTENSION_NAME = KHR_MATERIALS_VOLUME;
	...
	prewrite(context) {
		...
		this.document.getRoot().listMaterials().forEach((material) => {
			...
			if (volume) {
				...
				if (!_gltf_transform_core.MathUtils.eq(volume.getAttenuationColor(), [
					1,
					1,
					1
...
*/
var KHRMeshPrimitiveRestart = class extends _gltf_transform_core.Extension {
...
*/
var KHRMeshQuantization = class extends _gltf_transform_core.Extension {
...
*/
var Visibility = class extends _gltf_transform_core.ExtensionProperty {
...
* ```
*/
var KHRNodeVisibility = class extends _gltf_transform_core.Extension {
	static EXTENSION_NAME = KHR_NODE_VISIBILITY;
...
function isUncompressed(container) {
	return container.vkFormat > ktx_parse.VK_FORMAT_UNDEFINED && container.vkFormat <= ktx_parse.VK_FORMAT_E5B9G9R9_UFLOAT_PACK32;
}
function isUniversal(container) {
	const isBasisHDR = container.vkFormat === ktx_parse.VK_FORMAT_ASTC_4x4_SFLOAT_BLOCK_EXT && container.dataFormatDescriptor[0].colorModel === 167;
	return container.vkFormat === ktx_parse.VK_FORMAT_UNDEFINED || isBasisHDR;
}
var KTX2ImageUtils = class {
	...
	getSize(array) {
		const container = (0, ktx_parse.read)(array);
	...
	getChannels(array) {
		...
		if (isUniversal(container)) switch (dfd.colorModel) {
			case ktx_parse.KHR_DF_MODEL_ETC1S: return dfd.samples.length === 2 && (dfd.samples[1].channelType & 15) === 15 ? 4 : 3;
			case ktx_parse.KHR_DF_MODEL_UASTC: return (dfd.samples[0].channelType & 15) === 3 ? 4 : 3;
	...
	getVRAMByteLength(array) {
		...
		} else for (const level of container.levels) if (container.supercompressionScheme === ktx_parse.KHR_SUPERCOMPRESSION_NONE) uncompressedBytes += level.levelData.byteLength;
...
*/
var KHRTextureBasisu = class extends _gltf_transform_core.Extension {
	...
	/** @hidden */
	static register() {
		_gltf_transform_core.ImageUtils.registerFormat("image/ktx2", new KTX2ImageUtils());
	}
	/** @hidden */
	preread(context) {
...
*/
var Transform = class extends _gltf_transform_core.ExtensionProperty {
	static EXTENSION_NAME = KHR_TEXTURE_TRANSFORM;
	init() {
		this.extensionName = KHR_TEXTURE_TRANSFORM;
		this.propertyType = "Transform";
		this.parentTypes = [_gltf_transform_core.PropertyType.TEXTURE_INFO];
	}
	getDefaults() {
		return Object.assign(super.getDefaults(), {
			offset: [0, 0],
...
* ```
*/
var KHRTextureTransform = class extends _gltf_transform_core.Extension {
	...
	write(context) {
		...
		for (const [textureInfo, textureInfoDef] of textureInfoEntries) {
			...
			const transformDef = {};
			const eq = _gltf_transform_core.MathUtils.eq;
...
const PARENT_TYPES = [
	_gltf_transform_core.PropertyType.ROOT,
	_gltf_transform_core.PropertyType.SCENE,
...
*/
var Packet = class extends _gltf_transform_core.ExtensionProperty {
...
* ```
*/
var KHRXMP = class extends _gltf_transform_core.Extension {
		...
		for (const packet of this.properties) {
			...
			for (const parent of packet.listParents()) {
				...
				switch (parent.propertyType) {
					case _gltf_transform_core.PropertyType.ROOT:
						parentDef = json.asset;
						break;
					case _gltf_transform_core.PropertyType.SCENE:
						parentDef = json.scenes[context.sceneIndexMap.get(parent)];
						break;
					case _gltf_transform_core.PropertyType.NODE:
						parentDef = json.nodes[context.nodeIndexMap.get(parent)];
						break;
					case _gltf_transform_core.PropertyType.MESH:
						parentDef = json.meshes[context.meshIndexMap.get(parent)];
						break;
					case _gltf_transform_core.PropertyType.MATERIAL:
						parentDef = json.materials[context.materialIndexMap.get(parent)];
						break;
					case _gltf_transform_core.PropertyType.TEXTURE:
						parentDef = json.images[context.imageIndexMap.get(parent)];
						break;
					case _gltf_transform_core.PropertyType.ANIMATION:
...
exports.DiffuseTransmission = DiffuseTransmission;
...
exports.KHRAccessorFloat16 = KHRAccessorFloat16;
exports.KHRAccessorFloat64 = KHRAccessorFloat64;
...
exports.KHRMaterialsDiffuseTransmission = KHRMaterialsDiffuseTransmission;
exports.KHRMaterialsDispersion = KHRMaterialsDispersion;
...
exports.KHRMeshPrimitiveRestart = KHRMeshPrimitiveRestart;
exports.KHRMeshQuantization = KHRMeshQuantization;
exports.KHRNodeVisibility = KHRNodeVisibility;
...
exports.NodeStructuralMetadata = NodeStructuralMetadata;
