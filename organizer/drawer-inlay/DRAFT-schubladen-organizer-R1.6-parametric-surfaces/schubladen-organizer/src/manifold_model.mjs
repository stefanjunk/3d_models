...
function buildModule (tray, def, p, segments, withTexture) {
  ...
  return { def, smooth: shape, textured: shape, texture_stats: summarizeTextureStats(textureStats, p.surface_texture) }
}
...
function assertParameters (p) {
  ...
  const maximumSurfaceDepth = surface => surface.enabled ? surface.maximum_depth_mm : 0
  ...
  if (p.surface_texture.surfaces.outer_walls.enabled) throw new Error('R1.6 surface contract requires smooth outer walls')
