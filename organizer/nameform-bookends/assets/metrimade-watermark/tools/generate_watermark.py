...
def layout_metrics(font: TTFont, product_id: str, version: str) -> tuple[float, float, float, float]:
    height = 12.8
    text_x = 11.2
    domain_width = text_width_by_height(font, DOMAIN, 4.4)
    trace_text = f"{product_id} · v{version}"
    trace_width = text_width_by_height(font, trace_text, 3.6)
    width = round(text_x + max(domain_width, trace_width) + 1.0, 3)
    return width, height, domain_width, trace_width
...
module watermark_profile_core() {{
    union() {{
        translate([0.8, 0.8]) scale(11.2 / 238)
            import("{icon_filename}", center = false);
        translate([11.2, 7.4])
            fitted_text("{DOMAIN}", {domain_width:.5f}, 4.4);
        translate([11.2, 1.8])
            fitted_text("{product_id} · v{version}", {trace_width:.5f}, 3.6);
    }}
}}
...
def build(product_id: str, version: str, depth: float, font_path: Path, output_root: Path) -> Path:
    validate_inputs(product_id, version, depth)
    font = load_font(font_path)
    slug = f"{product_id}_v{version}"
    output_dir = output_root / slug
    output_dir.mkdir(parents=True, exist_ok=True)
    stem = f"metrimade-watermark-{product_id}-v{version}"

    width, height, domain_width, trace_width = layout_metrics(font, product_id, version)
    ...
    metadata = {
        ...
        "version": version,
        "visible_text": [DOMAIN, f"{product_id} · v{version}"],
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="Generate a metriMade watermark containing domain, product ID, and version."
    )
