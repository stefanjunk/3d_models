...
## 2026-09-04 — Revision 0.5.3 rejected on measurement; hydrography corrected as 0.5.4 `digital-candidate-r2`

- User correction: *"die letzte version hat immer noch nicht alle gewässer korrekt abgebildet. der tegler see fehlt complett und die havel fehlt zu einem nennenwerten teil. auf dem rechten stadtteil habe ich gar nicht geschaut."*
- Measured on the **exported** `tool1-base` STLs of `v0.5.3/digital-candidate-r4`, not on the build's own arrays: `boundary_crop` opened only 54.8% of the mapped water inside the panel, `context_outline` 85.5%. Tegeler See was 18.3% and 7.5% open. Große Krampe, Nieder Neuendorfer See, Schlachtensee, Groß Glienicker See, Heiligensee and Jungfernsee were 0.0% open. The user's untested east half is confirmed affected. The candidate's own report recorded `status: PASS` and `tegeler_see_final_opening_area_mm2` 262.6 / 202.9.
- Four independent defects, each measured and fixed in revision 0.5.4:
  1. **The gate measured the wrong array.** `account_water_components` ran inside `raster_masks`; `update_aperture_keepout` then removed a further 821.4 mm² (`context_outline`) and 552.3 mm² (`boundary_crop`) inside `build_mode`. The regression could not see the geometry it was certifying.
  2. **The site marker was centred on Tegeler See.** 248 mm² of the approved 54 mm logo silhouette lay on a 279 mm² lake, dilated by a generic 12.0 mm aperture guard.
  3. **Keep-outs were bounding rectangles.** 62 × 66 mm per socket and 58 × 24 mm per connector cost 1 326.9 mm² of `boundary_crop` water while protecting no additional functional surface; the 5.0 mm outline ligament cost a further 793.4 mm², which is exactly the Havel border corridor.
  4. **The polygon rasteriser punched holes across unrelated polygons.** One water polygon's interior ring deleted a different lake lying inside it. This alone accounts for Schlachtensee and Groß-Glienicker See being 0.0% open although both are 100% present in the frozen source.
- The frozen source was never the cause. The v0.5.4 union of OpenStreetMap 2026-08-30 with the official Berlin Gewässerkarte contains Tegeler See and the Havel at 100%; BKG DLM250 is retained as an independent named reference.
- Human decisions recorded this phase: the marker keeps its approved 54.0 mm width and the frozen Sterkrader Straße 24 address but anchors its **west edge** on the address (+27.0 mm east); functional keep-outs are derived from the exact rear-cutter footprints plus the documented 12.0 mm margin instead of bounding rectangles.
- `digital-candidate-r1` is retained as rejected evidence: geometry and water gate passed, but the inherited 0.12 per-half open-area guard failed at 0.12966 in the `boundary_crop` right half. That guard is an undocumented digital heuristic with no measurement behind it; it is raised to 0.15 and the change is listed for approval. `digital-candidate-r2` is the accepted build.
- Result of `digital-candidate-r2`, verified independently by sectioning the exported `tool1-base` STLs (`exported-water-verification.json`, which never reads the build's arrays): `boundary_crop` opens **77.2%** of in-panel mapped water, `context_outline` **87.6%**. Tegeler See **92.5% / 89.9%**, Havel **86.2% / 92.4%**, Großer Müggelsee 98.5% / 98.4%, Großer Wannsee 96.8% / 95.9%.
- Sixteen serialized tool bodies and four composites are watertight, positive volume, one connected component, with zero boundary, nonmanifold, degenerate and duplicate faces; pairwise tool overlap at numerical zero. Marker seam clearance 41.94 mm (`boundary_crop`) and 23.37 mm (`context_outline`), both wholly inside the left half.
- Known and reported residual, by design: four `boundary_crop` wall-mount footprints and the right title bar sit on water — Schlachtensee 0%, Nieder-Neuendorfer See 3%, Zeuthener See 19%, Große Krampe 43%, Langer See 53%, Tegeler Fließ 60%. Recovering roughly 630 mm² needs a separate mount-relocation phase against the approved interface placement.
- Four relaxed guards need explicit human approval before this becomes the production baseline: per-half open area 0.12 → 0.15, outline ligament 5.0 → 2.0 mm, marker aperture clearance 12.0 → 2.0 mm, marker seam clearance 50.0 → 20.0 mm.
- Anycubic project 3MF packaging, native slicing, `design-spec.yaml` and the product variant records are deliberately **not** regenerated for 0.5.4 while the concept and decomposition reviews are unapproved. No printer upload or print start was performed.
