...
|---|---|---|---|---|---|
| DEC-DECOMP-040 | resolved | Approve the revision 0.4.0 mode-aware component, source, interface, color and validation decomposition. | Stefan explicitly wrote 'freigegeben' on 2026-09-01 in direct response to the reviewed revision 0.4.0 decomposition. | Recorded human approval in design-spec.yaml, decomposition-review-0.4.0.md and decision-log.md. |  |
| DEC-SOURCE-040 | resolved | Freeze the expanded context_outline Berlin/Brandenburg source and derived layers. | source-data/v0.4.0/berlin/source-manifest.json freezes the verified Geofabrik/GWDG Germany-260830 transport derivative, proves strict context-bbox containment and records non-empty semantic layers. | Retain immutable snapshot/layer hashes and OpenStreetMap/Geofabrik attribution through packaging. |  |
| DEC-PLACEMENT-040 | resolved | Freeze mode-specific connector, hanger, standoff and LED-route placements. | validation/v0.4.0/berlin/interface-placement-report.json passes for both modes; every actual and protected interface footprint lies inside the retained body with at least 5.0 mm modeled outer ligament. | Physical process compensation, snap behavior and wall-load proof remain separate later gates. |  |
| DEC-PERIMETER-040 | provisional | Freeze boundary_crop perimeter simplification and weak-peninsula policy. | Digital candidate r9 preserves the administrative silhouette with 0.06 mm physical simplification, no positive area outside the mode mask, one connected backer per half and a matched top preview. | Physical print and handling review must confirm that retained peninsulas and outline edges survive the final process without unacceptable visual deviation. | physical, release |
| DEC-APERTURE-040 | provisional | Freeze mode-specific light-through path sets and rear halo routes. | Digital candidate r9 keeps every half connected after through-aperture subtraction, inserts only local 2.0 mm material bridges, stays below the 12% open-area limit and passes serialized topology plus exact target slicing. | A process-matched coupon and customer-selected lighting test must confirm bridge strength, transmitted-light character and rear-route usability. | physical, release |
...
| MAP_SOURCE_SET | hybrid | mixed | owns transport files, provenance, working-CRS derived layers and coverage evidence | [0, 0, 0] → [600, 400, 0.01] (600 × 400 × 0.01 mm) | not applicable / semantic source only | IF-SOURCE-MASK, IF-MAIN-MAP |
