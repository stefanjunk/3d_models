#!/usr/bin/env python3
"""Author one MM-ART-010 revision 0.5.3 half as an Anycubic 3MF."""
...
def load_engine():
    spec = importlib.util.spec_from_file_location("mm_art_010_package_v053_engine", PACKAGING_ENGINE)
...
def write_filament_profiles(
    ...
    for entry in colors:
        ...
        data = dict(base)
        profile_name = f"MM-ART-010 v0.5.3 Berlin {entry['name']}"
        ...
        data.update(
            {
                ...
                "from": "user",
                "is_custom_defined": "1",
                "setting_id": f"MMART010-053-{preset}-{index}",
                "filament_id": f"MMART010-053-{preset}-{index}",
...
def main() -> None:
    ...
    export_root = PRODUCT / "exports" / "v0.5.3" / "berlin" / args.candidate / args.mode
    ...
    with tempfile.TemporaryDirectory(prefix="mm-art-010-053-anycubic-") as temporary:
