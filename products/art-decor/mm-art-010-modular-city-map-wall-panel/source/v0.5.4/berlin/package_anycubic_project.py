#!/usr/bin/env python3
"""Author one MM-ART-010 revision 0.5.4 half as an Anycubic 3MF."""
...
PACKAGING_ENGINE = (
    PRODUCT / "source" / "v0.5.0" / "berlin" / "package_anycubic_site_marker_project.py"
)
PART_SUFFIXES = (
    "tool1-base",
    "tool2-relief",
...
def load_engine():
    spec = importlib.util.spec_from_file_location("mm_art_010_package_v054_engine", PACKAGING_ENGINE)
    if spec is None or spec.loader is None:
        raise RuntimeError(f"cannot load packaging engine: {PACKAGING_ENGINE}")
    module = importlib.util.module_from_spec(spec)
    spec.loader.exec_module(module)
    module.SEMANTIC_NAMES = [
        "Tool 1 Oak Land Base",
        "Tool 2 Mint Green Relief and Areas",
        "Tool 3 Midnight Street Network",
        "Tool 4 Sky Blue S/U Transit, Boundary and Site Marker",
    ]
    return module
...
def write_filament_profiles(
    ...
    for entry in colors:
        ...
        data = dict(base)
        profile_name = f"MM-ART-010 v0.5.4 Berlin {entry['name']}"
        ...
        data.update(
            {
                ...
                "from": "user",
                "is_custom_defined": "1",
                "setting_id": f"MMART010-054-{preset}-{index}",
                "filament_id": f"MMART010-054-{preset}-{index}",
...
def main() -> None:
    ...
    engine = load_engine()
    _, colors = engine.load_palette(args.palette)
    export_root = PRODUCT / "exports" / "v0.5.4" / "berlin" / args.candidate / args.mode
    ...
    mesh_metrics = [engine.serialized_mesh_metrics(path) for path in inputs]
    ...
    bed_min_x, bed_min_y, bed_max_x, bed_max_y = engine.parse_bed_bounds(args.machine_profile)
    source_min, source_max = engine.source_bounds(inputs)
    ...
    with tempfile.TemporaryDirectory(prefix="mm-art-010-054-anycubic-") as temporary:
        ...
        normalization = engine.normalize_project(
            raw_project,
            args.output,
            translate_x,
            translate_y,
    ...
    report = {
        ...
        "palette_catalog_sha256": sha256(engine.PALETTE_CATALOG_PATH),
        "status": "PASS",
        "representation": "native Anycubic project 3MF with four aligned, named, nonempty mesh volumes",
        "output": {
        ...
        "toolchain": {
            ...
            "packaging_engine": str(PACKAGING_ENGINE.resolve()),
            "packaging_engine_sha256": sha256(PACKAGING_ENGINE),
        ...
        "limitations": [
            ...
            "The generic Anycubic PLA Matte profile is provisional for the four SUNLU PLA-family rolls.",
