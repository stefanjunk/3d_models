#!/usr/bin/env python3
"""Run the canonical Anycubic G-code review with revision 0.5.4 metadata."""
...
ENGINE = PRODUCT / "source" / "v0.5.0" / "berlin" / "review_anycubic_site_marker_slice.py"
...
def main() -> None:
    ...
    with tempfile.TemporaryDirectory(prefix="mm-art-010-054-slice-review-") as temporary:
        intermediate = Path(temporary) / "engine-review.json"
        command = [
            "python", str(ENGINE),
            "--adapter-report", str(args.adapter_report),
            "--gcode", str(args.gcode),
            "--project-3mf", str(args.project_3mf),
            "--mode", args.mode,
            "--half", args.half,
            "--output", str(intermediate),
            "--expected-tools", args.expected_tools,
            "--minimum-tool-changes", str(args.minimum_tool_changes),
            "--maximum-tool-changes", str(args.maximum_tool_changes),
            "--expected-maximum-z", str(args.expected_maximum_z),
        ]
    ...
    report["revision"] = "0.5.4"
    report["reviewer"] = {
        "wrapper": str(Path(__file__).resolve()),
        "wrapper_sha256": sha256(Path(__file__).resolve()),
        "review_engine": str(ENGINE.resolve()),
        "review_engine_sha256": sha256(ENGINE),
    }
    report["limitations"].append(
        "Physical bridge strength and installed wall proof-load remain unqualified."
    )
    report["limitations"].append(
        "Known wall-mount residuals (Schlachtensee, Nieder-Neuendorfer See, Zeuthener See, Große Krampe, Langer See) accepted by owner 2026-09-05."
    )
