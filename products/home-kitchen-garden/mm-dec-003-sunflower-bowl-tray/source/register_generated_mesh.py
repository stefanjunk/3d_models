#!/usr/bin/env python3
"""Select the intended Step1X shell and register it in millimetres."""
...
def main() -> int:
    ...
    parser.add_argument("--target-longest-xy-mm", type=float, required=True)
    parser.add_argument("--report", type=Path, required=True)
    args = parser.parse_args()

    source_path = args.input.resolve()
    target = float(args.target_longest_xy_mm)
    if target <= 0 or not math.isfinite(target):
        raise SystemExit("--target-longest-xy-mm must be finite and positive")
    ...
    components = sorted(flattened.split(only_watertight=False), key=lambda mesh: len(mesh.faces), reverse=True)
    if not components:
        raise SystemExit("input contains no mesh component")
    selected = components[0].copy()
    if not selected.is_watertight or not selected.is_volume:
        raise SystemExit("largest component is not a positive watertight solid")

    selected.apply_transform(trimesh.transformations.rotation_matrix(math.pi / 2.0, [1.0, 0.0, 0.0]))
    scale = target / float(max(selected.extents[0], selected.extents[1]))
    ...
    report = {
        ...
        "operation": "largest intended shell selection plus uniform XY metric registration",
        ...
        "selection_rule": "largest positive watertight component by face count; rejected components remain only in the immutable raw GLB",
        "transform": {
            "orientation": "glTF +Y-up rotated +90 degrees about X to project +Z-up",
            "uniform_scale_factor_to_mm": scale,
            "target_longest_xy_mm": target,
