...
def render_overview(components: list[dict], output: Path) -> None:
    ...
    configure_axes(ax, "Regenwasser-Filterbrunnen · Revision 2")
    ...
    fig.text(
        0.5,
        0.025,
        "DRAFT CAD rendering · 816 mm tower height · detachable cascade",
        ha="center",
        fontsize=9,
        color="#475569",
    )
...
def exploded_shift(name: str) -> np.ndarray:
    ...
    if name == "Zulaufadapter 25 mm":
        return np.array([-180.0, 0.0, 165.0])
...
def main() -> int:
    ...
    render_overview(components, args.output_dir / "DRAFT_R2_assembly_overview.png")
    render_cutaway(components, args.output_dir / "DRAFT_R2_assembly_cutaway.png")
    render_exploded(components, args.output_dir / "DRAFT_R2_assembly_exploded.png")
