"""R3 refinement: two continuous ribbed petals, not a cut wavy vessel rim.
...
def envelope(p, na=None, nz=None):
    ...
    def xyz(z, u, angle_material=None, petal_s=None, inner=False, petal=0):
        h = z / p["height_mm"]
        ...
        ridge = (.5+.5*math.cos(p["rib_count"]*u))**p["rib_crest_power"]
        ...
        drift = 0 if petal_s is None else float(smoothstep(petal_s))
        dx = p["petal_tip_dx_mm"][petal]*drift
        dy = p["petal_tip_dy_mm"][petal]*drift
        return ((r*math.cos(angle)+float(center_curve(h)))*p["width_mm"]/92+dx,
                r*math.sin(angle)*p["depth_mm"]/92+dy,z)
    ...
    for petal in (0,1):
        ...
        for inner in (False,True):
            ...
            for j in range(1,petal_n):
                ...
                halfwidth=(math.pi/2)*math.sqrt(max(0,1-s**p["petal_convergence_exponent"]))
                row=[]
                for q in qs:
                    row.append(len(vertices))
                    vertices.append(xyz(z,center+q*math.pi/2,
                                        center+q*halfwidth,s,inner,petal))
                rows.append(row)
        ...
        tip=len(vertices)
        vertices.append(xyz(top_z,center,center,1.0,False,petal))
