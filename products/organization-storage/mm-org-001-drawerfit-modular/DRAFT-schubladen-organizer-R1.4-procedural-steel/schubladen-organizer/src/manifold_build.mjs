...
function baseReport (params, paramsPath, texturePath, preview, markOutline) {
  return {
    status: 'DRAFT',
...
function main () {
  ...
  const texturePath = path.resolve(path.dirname(paramsPath), rawParams.surface_texture.config)
  const params = resolveModelParameters({ ...rawParams, surface_texture: readJson(texturePath) })
  ...
  console.error(`Building ${params.model_revision} with ${params.surface_texture.representation}; seed ${params.surface_texture.seed}`)

  const report = baseReport(params, paramsPath, texturePath, preview, markOutline)
  ...
  const accessoryEntries = [
    ...
    ['steel_texture_coupon', textureCoupon, 'DRAFT-steel-texture-coupon.stl'],
