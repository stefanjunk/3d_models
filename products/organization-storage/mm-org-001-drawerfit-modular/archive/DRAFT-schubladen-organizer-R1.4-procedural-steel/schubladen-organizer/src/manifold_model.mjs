...
function innerWallTexturePatches (def, p) {
  ...
  const addX = (x, normal, y0, y1, top, key, keepouts = []) => {
    ...
    patches.push({ u0: y0, u1: y1, v0: bottom, v1: z1, key, normal: [normal, 0, 0], keepouts, pointAt: (y, z, offset) => [x + normal * offset, y, z] })
  }
  const addY = (y, normal, x0, x1, top, key, keepouts = []) => {
    const z1 = top - t.surfaces.inner_walls.top_keepout_mm
    if (x1 - x0 <= 2 || z1 - bottom <= 2) return
    patches.push({ u0: x0, u1: x1, v0: bottom, v1: z1, key, normal: [0, normal, 0], keepouts, pointAt: (x, z, offset) => [x, y + normal * offset, z] })
  }
...
function wallTopPatch (rect, z, key, globalKeepouts = []) {
  ...
  return alongX
    ? { u0: 0, u1: widthX, v0: 0, v1: widthY, key, normal: [0, 0, 1], keepouts: toLocal, pointAt: (u, v, offset) => [rect.x0 + u, rect.y0 + v, z + offset] }
    : { u0: 0, u1: widthY, v0: 0, v1: widthX, key, normal: [0, 0, 1], keepouts: toLocal, pointAt: (u, v, offset) => [rect.x0 + v, rect.y0 + u, z + offset] }
}
...
function assertParameters (p) {
  ...
  const floorDepth = Math.max(...p.surface_texture.surfaces.floor.bands.flatMap(item => item.depth_mm))
  const wallDepth = Math.max(...p.surface_texture.surfaces.inner_walls.bands.flatMap(item => item.depth_mm))
  ...
  if (p.surface_texture.surfaces.outer_walls.enabled) throw new Error('R1.4 approved concept requires smooth outer walls')
...
export function buildTextureCouponManifold (p) {
  ...
  const stats = []
  const floorPatches = [
    { x0: 18, x1: 40, scale: 0.75, name: 'low' },
    { x0: 41, x1: 65, scale: 1.0, name: 'approved' },
    { x0: 66, x1: 88, scale: 1.20, name: 'high' }
  ].map(item => ({
    u0: item.x0,
    u1: item.x1,
    v0: 2,
