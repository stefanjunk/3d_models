...
def main() -> None:
    ...
    drawer_depth_stack = drawer["body_depth"] + drawer["rear_clearance"]
    ...
    checks = [
        ...
        check("drawer-depth-stack", close(drawer_depth_stack, cavity_depth), "Drawer body and rear reserve close the housing cavity depth exactly; the rounded fascia remains proud of the housing.", {"drawer_stack_mm": drawer_depth_stack, "cavity_mm": cavity_depth}),
        ...
        check("assembly-envelope", all(close(a, b, 0.05) for a, b in zip(assembly_extents, [housing["width"], housing["depth"] + drawer["front_depth"], housing["height"] + sorter["height"]])), "Assembled envelope includes the proud drawer fascia.", {"extents_mm": assembly_extents}),
    ]
    payload = {
        "schema_version": "1.0",
        ...
        "limitations": [
            ...
            "No qualified process record exists for the declared 0.45 mm side clearance.",
