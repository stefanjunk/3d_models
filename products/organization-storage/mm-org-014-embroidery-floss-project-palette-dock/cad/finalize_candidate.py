#!/usr/bin/env python3
"""Create the hash-bound digital print-candidate report for MM-ORG-014."""
...
REPORT_PATHS = [
    ...
    "validation/fdm-mesh-coupon.json",
...
def main() -> None:
    ...
    checks = [
        check(f"report:{name}", report.get("status") == "PASS", f"{name} reports PASS")
    ...
    checks.extend(
        [
            check("gcode-report", valid_gcode, "Exactly one G-code analysis reports PASS"),
            check(
            ...
            check(
                ...
                True,
                "Actual card fit, fiber snagging, stability and cycle tests remain outside this digital candidate",
            ),
            check(
                "watermark-gate-deferred",
                True,
                "Final watermark and commercial release remain blocked",
            ),
        ]
    )
    ...
    output = {
        "schema_version": "1.0",
        "tool": "MM-ORG-014-finalize-digital-candidate",
        ...
        "limitations": [
            "Declared card classes are dimensional envelopes until checked with actual loaded floss cards.",
            "Headless slicing does not replace layer-preview inspection or a physical print.",
            "Open receivers are not needle storage and do not provide portable retention.",
