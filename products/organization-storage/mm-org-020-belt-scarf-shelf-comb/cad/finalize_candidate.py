#!/usr/bin/env python3
"""Create the hash-bound digital print-candidate report for MM-ORG-020."""
...
REPORT_PATHS = [
    ...
    "validation/fdm-mesh-belt-four.json", "validation/fdm-mesh-scarf-three.json",
    "validation/fdm-mesh-edge-radius-coupon.json", "validation/fdm-mesh-connector-key.json",
...
def main() -> None:
    ...
    checks.extend([
        ...
        check("two-hundred-ninety-layers", metrics.get("layers_from_comments") == 290, "58.0 mm divider height slices as 290 layers at 0.20 mm"),
        ...
        check("shared-connector-datums", interface.get("belt-four", {}).get("joint_centers_y_mm") == interface.get("scarf-three", {}).get("joint_centers_y_mm") == [28.0, 77.0], "Both comb presets share connector datums"),
        check("production-edge-radius", interface.get("belt-four", {}).get("divider_leading_radius_mm") == interface.get("scarf-three", {}).get("divider_leading_radius_mm") == 1.4, "Both production presets retain R1.4 textile-facing noses"),
        check("physical-gates-deferred", True, "Fabric snag, roll retention, connector fit, retrieval cycling and shelf sliding remain user-owned"),
    ])
    output = {
        "schema_version": "1.0", "tool": "MM-ORG-020-finalize-digital-candidate", "tool_version": REVISION,
        ...
        "metrics": {
            "build_sets": 1, "unique_meshes": 4, "objects": 4, "layers": metrics.get("layers_from_comments"),
        ...
        "limitations": [
            "Roll dimensions are example measured envelopes, not compatibility claims for every belt or scarf.",
            "Headless slicing does not replace final layer preview, coupon selection or a physical print.",
            "Fabric snagging, fringe behavior, connector clearance, retention, 100 retrieval cycles and shelf sliding are untested.",
