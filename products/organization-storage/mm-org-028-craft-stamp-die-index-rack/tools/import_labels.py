#!/usr/bin/env python3
"""Validate a category CSV and emit hash-bound normalized divider data."""
...
def parse_rows(csv_path: Path, parameters: dict) -> list[dict]:
    ...
    with csv_path.open(encoding="utf-8-sig", newline="") as stream:
        ...
        for index, row in enumerate(reader, 1):
            ...
            data = layout(label, divider["tab_width_mm"] - 2.0 * divider["text_margin_x_mm"], divider["text_height_mm"], divider["maximum_pixel_pitch_mm"], divider["minimum_pixel_width_mm"])
            rows.append({"index": index, "source_label": row["label"], "normalized_label": label, "tab_position": position, "tab_center_x_mm": divider["width_mm"] / 2.0 + divider["tab_offsets_x_mm"][position], "layout": data})
...
def main() -> None:
    ...
    checks = [
        {"id": "label-count", "status": "PASS", "required": True, "message": f"Imported {len(rows)} unique category labels", "metrics": {"count": len(rows)}, "evidence": []},
    ...
    report = {"schema_version": "1.0", "tool": "MM-ORG-028-csv-label-import", "tool_version": parameters["project"]["revision"], "status": "PASS", "profile": "draft", "inputs": [record(args.parameters), record(args.csv), record(ROOT / "cad/gridfont.py")], "checks": checks, "metrics": {"font_id": FONT_ID, "labels": rows, "batch_json_sha256": sha256(args.json_out)}, "limitations": ["Customer labels still require an exact proof and rights/content review before manufacture."], "required_capabilities": []}
