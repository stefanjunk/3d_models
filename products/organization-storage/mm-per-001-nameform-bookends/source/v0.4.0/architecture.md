# NameForm 0.4.0 production architecture
...
- CadQuery owns the exact functional core: side blade, inward foot, ribs, gussets,
  6.0 mm glyph bodies, and the 2.4 mm recessed rear connector.
...
- The rear connector repeats each glyph silhouette at `y=5.9..8.4 mm` and adds
  only 6.0 mm wide local bridges between nearest neighboring outlines and the
  side blade. The 0.1 mm overlap is the controlled bond to the 6.0 mm glyph body.
  There is no rectangular facade panel.
...
- the same 6.0 mm body depth, 2.4 mm connector depth, edge taper, orientation,
...
- `validation/v0.4.0/`: generation report, mesh audits, exact pair slice evidence,
