#!/usr/bin/env python3
"""Build personalized reinforced NameForm 0.4.1 pairs and the FA process coupon.
...
os.environ.setdefault(
    "MPLCONFIGDIR", str(Path(tempfile.gettempdir()) / "nameform-v041-matplotlib")
)
...
GLYPH_DEPTH = 12.0
CONNECTOR_FRONT_Y = 10.0
CONNECTOR_T = 4.0
CONNECTOR_REAR_Y = CONNECTOR_FRONT_Y + CONNECTOR_T
CONNECTOR_OVERLAP = GLYPH_DEPTH - CONNECTOR_FRONT_Y
CONNECTOR_EXTENSION_BEHIND_GLYPH = CONNECTOR_REAR_Y - GLYPH_DEPTH
BRIDGE_WIDTH = 12.0
MIN_CONNECTOR_OVERLAP = 2.0
MIN_BRIDGE_WIDTH = 12.0
...
def validate_reinforcement_contract() -> None:
    """Fail before expensive geometry if the approved depth contract drifts."""

    checks = {
        "glyph_depth": math.isclose(GLYPH_DEPTH, 12.0, abs_tol=1.0e-9),
        "connector_front": math.isclose(CONNECTOR_FRONT_Y, 10.0, abs_tol=1.0e-9),
        "connector_thickness": math.isclose(CONNECTOR_T, 4.0, abs_tol=1.0e-9),
        "connector_overlap": CONNECTOR_OVERLAP >= MIN_CONNECTOR_OVERLAP,
        "connector_rear_extension": math.isclose(
            CONNECTOR_EXTENSION_BEHIND_GLYPH, 2.0, abs_tol=1.0e-9
        ),
        "local_bridge_width": BRIDGE_WIDTH >= MIN_BRIDGE_WIDTH,
    }
    failed = [name for name, passed in checks.items() if not passed]
    if failed:
        raise ValueError(f"reinforcement contract failed: {', '.join(failed)}")
...
def build_coupon_brep() -> tuple[cq.Workplane, list[PlacedGlyph], Polygon | MultiPolygon, list[dict]]:
    validate_reinforcement_contract()
    ...
    parts.extend(profile_solids(profile, CONNECTOR_FRONT_Y, CONNECTOR_T))
...
def write_binary_stl_once(path: Path, vertices: np.ndarray, faces: np.ndarray) -> None:
    ...
    header = b"MM-PER-001 NameForm letter-only wood C v0.4.1 DRAFT"[:80].ljust(80, b" ")
    path.parent.mkdir(parents=True, exist_ok=True)
...
def engineering_report(
    ...
    checks = {
        ...
        "approved_glyph_depth": math.isclose(GLYPH_DEPTH, 12.0, abs_tol=1.0e-9),
        "approved_connector_front": math.isclose(
            CONNECTOR_FRONT_Y, 10.0, abs_tol=1.0e-9
        ),
        "approved_connector_thickness": math.isclose(
            CONNECTOR_T, 4.0, abs_tol=1.0e-9
        ),
        "minimum_positive_depth_overlap": CONNECTOR_OVERLAP
        >= MIN_CONNECTOR_OVERLAP,
        "minimum_local_bridge_width": all(
            bridge["width_mm"] >= MIN_BRIDGE_WIDTH for bridge in bridges
        ),
        "connector_recessed_behind_front": CONNECTOR_FRONT_Y > WOOD_DEPTH,
    }
    return {
        ...
        "connector": {
            "front_y_mm": CONNECTOR_FRONT_Y,
            "rear_y_mm": CONNECTOR_REAR_Y,
            "positive_depth_overlap_mm": CONNECTOR_OVERLAP,
            "extension_behind_glyph_mm": CONNECTOR_EXTENSION_BEHIND_GLYPH,
            "thickness_mm": CONNECTOR_T,
            "minimum_bridge_width_mm": MIN_BRIDGE_WIDTH,
            "gross_bridge_section_mm2": BRIDGE_WIDTH * CONNECTOR_T,
            "historical_gross_bridge_section_mm2": 6.0 * 2.4,
            "gross_section_ratio_vs_v0_4_0": (BRIDGE_WIDTH * CONNECTOR_T)
            / (6.0 * 2.4),
...
def build_coupon(sampler: PeriodicSampler, wood: dict) -> dict:
    coupon_root = PROJECT_ROOT / "coupons/nameform-letter-bridge-v0.4.1"
    engineering_path = coupon_root / "exports/engineering/nameform-FA-bridge-v0.4.1.step"
    candidate_path = coupon_root / "exports/candidate/DRAFT-nameform-FA-wood-C-v0.4.1.stl"
...
def build_pair(
    ...
    report = {
        ...
        "limitations": [
            ...
            "The 0.4.1 product watermark is not generated or placed yet.",
            "Exact filament product, color, batch and conditioning remain unresolved.",
            "No printer upload or print start is performed.",
        ],
    }
