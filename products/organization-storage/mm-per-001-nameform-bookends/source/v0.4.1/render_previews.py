#!/usr/bin/env python3
"""Render fixed front and grazing-light previews for NameForm 0.4.1 variants."""
...
def render_pair_rear(
    ...
    """Render the recessed connector from the rear as structural evidence."""
    bpy.ops.wm.read_factory_settings(use_empty=True)
    prepare_pair(left_path, right_path, left_label, right_label)
    target = (10.0, 12.0, 78.0)
    add_area((-330.0, 170.0, 245.0), 5200.0, 78.0, target)
    add_area((360.0, 125.0, 150.0), 3300.0, 64.0, target)
    add_area((0.0, -185.0, 270.0), 2200.0, 110.0, target)
    add_sun((0.84, 0.44, 0.88), 3.2, 0.035)
    bpy.ops.object.camera_add(location=(10.0, 950.0, 120.0))
    camera = bpy.context.object
    camera.data.type = "ORTHO"
    camera.data.ortho_scale = 800.0
    camera.data.clip_end = 5000.0
...
def main() -> None:
    ...
    parser.add_argument("--pair-rear-output", type=Path, required=True)
