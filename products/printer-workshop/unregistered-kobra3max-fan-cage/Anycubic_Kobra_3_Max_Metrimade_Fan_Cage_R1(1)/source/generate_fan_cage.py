#!/usr/bin/env python3
"""Generate the original Metrimade Kobra 3 Max wraparound printhead-cover package.

The add-on cover combines a full-face honeycomb carrier, a shallow perimeter
shell, four side stabilizer clips, and the previously developed perforated-M
fan badge. Geometry is built as a deterministic
0.20 mm voxel solid.  This avoids a third-party CAD/mesh dependency while
preserving exact, auditable material partitions for the first-layer
multicolor inlay.
"""
...
Y_MIN, Y_MAX = -55.0, 39.0
Z_MIN, Z_MAX = 0.0, 10.80
...
LABEL_CENTER_Y_MM = -38.20
LABEL_WIDTH_MM = 54.0
LABEL_HEIGHT_MM = 8.80
COVER_FRONT_THICKNESS_MM = 1.60
BADGE_FRONT_THICKNESS_MM = 2.40
COVER_FRAME_MM = 2.00
PERIMETER_SHELL_DEPTH_MM = 4.80
SIDE_CLIP_DEPTH_MM = 10.80
SHELL_EDGE_NOMINAL_WIDTH_MM = 69.0
FAN_MODULE_OUTER_DIAMETER_MM = 59.20
FAN_MODULE_INNER_DIAMETER_MM = 50.80
HONEYCOMB_RADIUS_MM = 4.20
HONEYCOMB_RIB_MM = 1.20
...
def polygon_mask(
    ...
    """Rasterize a project-space polygon on the deterministic XY grid."""
    points_px = [
        ((x - X_MIN) / PITCH, (y - Y_MIN) / PITCH)
        for x, y in points_mm
    ]
    image = Image.new("1", (nx, ny), 0)
    ImageDraw.Draw(image).polygon(points_px, fill=1)
    return np.asarray(image, dtype=bool)


def honeycomb_mask(xx: np.ndarray, yy: np.ndarray) -> np.ndarray:
    """Coarse flat-top hexagon edges, modeled explicitly for portable slicing."""
    radius = HONEYCOMB_RADIUS_MM
    step_x = 1.5 * radius
    step_y = math.sqrt(3.0) * radius
    lattice = np.zeros_like(xx, dtype=bool)
    column = 0
    center_x = X_MIN - 2.0 * radius
    while center_x <= X_MAX + 2.0 * radius:
        offset_y = 0.5 * step_y if column % 2 else 0.0
        center_y = Y_MIN - 2.0 * radius + offset_y
        while center_y <= Y_MAX + 2.0 * radius:
            vertices = [
                (
                    center_x + radius * math.cos(math.radians(60.0 * index)),
                    center_y + radius * math.sin(math.radians(60.0 * index)),
                )
                for index in range(6)
            ]
            for index in range(6):
                lattice |= segment_mask(
                    xx,
                    yy,
                    vertices[index],
                    vertices[(index + 1) % 6],
...
def make_xy_masks(xs: np.ndarray, ys: np.ndarray) -> tuple[dict[str, np.ndarray], dict]:
    ...
    frame_cells = int(round(COVER_FRAME_MM / PITCH))
    outer_frame = outer_outline & ~_erode_pixel_mask(outer_outline, frame_cells)

    fan_keepout = rr < FAN_MODULE_OUTER_DIAMETER_MM / 2.0 - 0.40
    lattice = honeycomb_mask(xx, yy) & outer_outline & ~fan_keepout

    outer_ring = (
        (rr <= FAN_MODULE_OUTER_DIAMETER_MM / 2.0)
        & (rr >= FAN_MODULE_INNER_DIAMETER_MM / 2.0)
    )
    ...
    # Deterministic short bridges tie the fan badge to the full-cover carrier.
    surround_links = np.zeros_like(rr, dtype=bool)
    for a, b in (
        ((-28.8, 0.0), (-35.4, 0.0)),
        ((28.8, 0.0), (35.4, 0.0)),
        ((0.0, -28.8), (0.0, -34.0)),
        ...
        surround_links |= segment_mask(xx, yy, a, b, 1.60)
    ...
    cover = manifold_pixel_mask(outer_frame | lattice | surround_links)
    fan_badge = manifold_pixel_mask(outer_ring | spokes | brand_union | label_plate)
    front = manifold_pixel_mask(cover | fan_badge)

    surround_region = outer_outline & ~fan_keepout & ~label_plate
    surround_blocked = cover & surround_region
    surround_open_percent = 100.0 * (
        1.0 - surround_blocked.sum() / max(int(surround_region.sum()), 1)
    )

    top_stabilizer = outer_outline & (yy <= -48.0)
    ...
    return {
        ...
        "surround_links": surround_links,
        "top_stabilizer": top_stabilizer,
        **brand,
    }, {
        **brand_meta,
        "cover": {
            "photo_scaled_face_envelope_mm": [72.0, 88.0],
            "outer_shell_envelope_mm": [74.4, 88.0, SIDE_CLIP_DEPTH_MM],
            "outer_polygon_mm": [
                [-30.0, -52.0], [30.0, -52.0], [36.0, -48.0],
                [36.0, 29.0], [32.0, 36.0], [-32.0, 36.0],
                [-36.0, 29.0], [-36.0, -48.0],
            ],
            "frame_mm": COVER_FRAME_MM,
            "perimeter_shell_depth_mm": PERIMETER_SHELL_DEPTH_MM,
            "side_clip_depth_mm": SIDE_CLIP_DEPTH_MM,
            "side_clip_nominal_shell_width_mm": SHELL_EDGE_NOMINAL_WIDTH_MM,
            "honeycomb_radius_mm": HONEYCOMB_RADIUS_MM,
            "honeycomb_rib_mm": HONEYCOMB_RIB_MM,
            "honeycomb_nominal_cell_pitch_mm": [
                round(1.5 * HONEYCOMB_RADIUS_MM, 3),
                round(math.sqrt(3.0) * HONEYCOMB_RADIUS_MM, 3),
            ],
            "projected_surround_open_area_percent": round(surround_open_percent, 2),
        },
    }
...
def side_clip_mask(xx: np.ndarray, yy: np.ndarray, z: float) -> np.ndarray:
    """Four shallow wraparound stabilizers inferred from the stock cover's side-release method.

    The fan bezel remains the primary datum. These fingers suppress rotation and
    edge flutter; their compliant rear beads are intentionally small so they can
    flex or be trimmed if the photo-derived shell width differs at the printer.
    """
    half_shell = SHELL_EDGE_NOMINAL_WIDTH_MM / 2.0
    if z < 7.80:
        inner_x = half_shell + 0.55
    elif z < 9.20:
        inner_x = half_shell + 0.20
    elif z < 10.00:
        inner_x = half_shell - 0.10
    else:
        t = min(max((z - 10.00) / 0.80, 0.0), 1.0)
        inner_x = (half_shell - 0.10) * (1.0 - t) + (half_shell + 0.80) * t
    y_bands = ((yy >= -41.0) & (yy <= -29.0)) | ((yy >= 15.0) & (yy <= 27.0))
    return y_bands & (np.abs(xx) >= inner_x) & (np.abs(xx) <= 37.20)
...
def make_occupancy(target_diameter: float, fit_coupon: bool = False) -> tuple[dict[str, np.ndarray], dict]:
    ...
    if fit_coupon:
        coupon_front = manifold_pixel_mask(
            masks["outer_frame"] | masks["outer_ring"] | masks["surround_links"]
        )
        for k, z in enumerate(zs):
            ...
            if 1.00 <= z < PERIMETER_SHELL_DEPTH_MM:
                cage[k] |= masks["outer_frame"]
                cage[k] |= masks["top_stabilizer"]
            if 1.00 <= z < SIDE_CLIP_DEPTH_MM:
                cage[k] |= side_clip_mask(xx, yy, z)
        return {"single": cage}, {
            "pitch_mm": PITCH,
            "target_bezel_diameter_mm": target_diameter,
            "fit_frame": True,
            "tested_face_envelope_mm": [72.0, 88.0],
            "tested_shell_edge_width_mm": SHELL_EDGE_NOMINAL_WIDTH_MM,
        }

    for k, z in enumerate(zs):
        if z < COVER_FRONT_THICKNESS_MM:
            cage[k] |= masks["front"]
        elif z < BADGE_FRONT_THICKNESS_MM:
            cage[k] |= masks["fan_badge"]
        if 1.40 <= z < PERIMETER_SHELL_DEPTH_MM:
            cage[k] |= masks["outer_frame"]
            cage[k] |= masks["top_stabilizer"]
        if 2.20 <= z < 6.60:
            inner = clip_inner_radius(z, target_diameter)
            cage[k] |= sector & (rr >= inner) & (rr <= 28.40)
        if 1.40 <= z < SIDE_CLIP_DEPTH_MM:
            cage[k] |= side_clip_mask(xx, yy, z)
    ...
    return {
        ...
        "target_bezel_diameter_mm": target_diameter,
        "fit_frame": False,
        "cover_front_thickness_mm": COVER_FRONT_THICKNESS_MM,
        "badge_front_thickness_mm": BADGE_FRONT_THICKNESS_MM,
        "full_cover_envelope_mm": [74.4, 88.0, SIDE_CLIP_DEPTH_MM],
        "estimated_petg_mass_g": round(float(cage.sum()) * PITCH**3 * 1.27 / 1000.0, 2),
...
def write_3mf(path: Path, parts: list[tuple[str, str, Mesh]]) -> None:
    ...
    model = (
        ...
        '<metadata name="Title">Metrimade Kobra 3 Max Honeycomb Printhead Cover</metadata>'
        ...
        f'{"".join(objects_xml)}'
        f'<object id="{assembly_id}" name="printhead_cover_assembly" type="model"><components>{"".join(component_xml)}</components></object>'
        '</resources>'
...
def build(
    output_root: Path,
    diameters: tuple[float, ...] = (50.0, 52.0, 54.0),
    report_name: str = "generator-mesh-report.json",
) -> dict:
    ...
    summary: dict = {
        ...
        "limitations": [
            ...
            "Published manufacturer evidence does not include the raised bezel diameter or a dimensioned shell drawing.",
            "The cover is a photo-scaled independent add-on shell, not a replacement or trace of the manufacturer shell.",
            "The four side stabilizers use a provisional 69.0 mm shell-edge width and must be proven with the fit frame.",
        ],
        "variants": {},
    }
    ...
    for diameter in diameters:
        ...
        single_path = exports / f"printhead_cover_singlecolor_D{int(diameter)}.stl"
        write_binary_stl(single_path, single_mesh, f"printhead_cover_singlecolor_D{int(diameter)}")
        ...
        coupon_path = exports / f"printhead_cover_fit_frame_D{int(diameter)}.stl"
        write_binary_stl(coupon_path, coupon_mesh, f"printhead_cover_fit_frame_D{int(diameter)}")
        ...
        for name in ("body_navy", "brand_teal", "brand_aqua", "brand_sand"):
            ...
            part_path = exports / f"printhead_cover_D{int(diameter)}_{name}.stl"
        ...
        three_mf_path = exports / f"printhead_cover_metrimade_D{int(diameter)}_multicolor.3mf"
...
def merge_reports(output_root: Path, report_names: list[str], report_name: str) -> dict:
    reports_dir = output_root / "reports"
    documents = [json.loads((reports_dir / name).read_text(encoding="utf-8")) for name in report_names]
    if not documents:
        raise ValueError("At least one generator report is required for merging")
    merged = json.loads(json.dumps(documents[0]))
    merged["variants"] = {}
    for key in list(merged):
        if key.startswith("multicolor_D"):
            del merged[key]
    for document in documents:
        if document.get("inputs") != merged.get("inputs"):
            raise ValueError("Cannot merge generator reports with different source inputs")
        merged["variants"].update(document.get("variants", {}))
        for key, value in document.items():
            if key.startswith("multicolor_D"):
                merged[key] = value
    merged["merged_from"] = report_names
...
def main() -> None:
    ...
    parser.add_argument("--diameters", default="50,52,54")
    parser.add_argument("--report-name", default="generator-mesh-report.json")
    parser.add_argument("--merge-reports", nargs="*")
    args = parser.parse_args()
    output_root = args.output_root.resolve()
    if args.merge_reports:
        summary = merge_reports(output_root, args.merge_reports, args.report_name)
    else:
        diameters = tuple(float(value) for value in args.diameters.split(",") if value.strip())
        summary = build(output_root, diameters, args.report_name)
    print(json.dumps({
        "status": "PASS",
        "report": f"reports/{args.report_name}",
