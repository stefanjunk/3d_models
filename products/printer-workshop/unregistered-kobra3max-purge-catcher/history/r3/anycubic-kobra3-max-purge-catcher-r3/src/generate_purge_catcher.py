#!/usr/bin/env python3
"""Generate the R3 Anycubic Kobra 3 Max compact purge catcher project.
...
def write_binary_stl(mesh: Mesh, path: Path):
    ...
    header = f"metriMade R3 {mesh.name}".encode("ascii", "replace")[:80].ljust(80, b" ")
...
def make_catcher(params: dict, masks: dict):
    ...
    # The two side walls carry the lightweight honeycomb treatment.  The wall
    # opposite the spring-loaded purge paddle stays solid because it is the
    # primary impact surface.
    honeycomb_ribs = side_band & honey_side[None, :, :]

    hood_start = float(params["impact_hood_start_z_mm"])
    hood_depth = float(params["impact_hood_depth_mm"])
    hood_height = front_h - hood_start
    hood_t = np.clip((z - hood_start) / hood_height, 0.0, 1.0)
    hood_offset = hood_depth * hood_t * hood_t * (3.0 - 2.0 * hood_t)
    hood_outer_y = -upper_d / 2.0 + hood_offset
    impact_hood = (
        (z >= hood_start)
        & (z <= front_h)
        & (np.abs(x) <= upper_w / 2.0)
        & (y >= hood_outer_y)
        & (y <= hood_outer_y + wall + pitch / 2.0)
    )
    ...
    upper_keep = containment_skin | honeycomb_ribs | top_frames | edge_frames | transition_frame | logo_support | rear_band | front_band
    occupancy = (shell & height_mask & (low_funnel | upper_keep)) | impact_hood
    baseline_occupancy |= impact_hood
    ...
    slot = np.zeros_like(occupancy)
    for slot_center_z in (pair_center - pair_spacing / 2.0, pair_center + pair_spacing / 2.0):
        slot_dz = np.maximum(np.abs(z - slot_center_z) - straight_half, 0.0)
        slot |= (x * x + slot_dz * slot_dz <= slot_radius * slot_radius) & (y >= upper_d / 2.0 - pitch / 2.0)
    removed_slot_voxels = int(np.count_nonzero(occupancy & slot))
    baseline_occupancy &= ~slot
    occupancy &= ~slot
    allowed = ((shell & height_mask) | plate | impact_hood) & ~slot
    ...
    return body, occupancy, {
        ...
        "mount_slot_bridge_mm": slot_width,
        "mount_slot_axis": "vertical pair",
...
def save_assembly_preview(path: Path, catcher: Mesh, logos: dict, lower_bin: Mesh):
    ...
    # Functional side section: the wiper sits at the right/rear and ejects
    # toward the tall wall at the left/front.  The top 16 mm roll inward.
    catcher_section = [
        (22, 123), (22, 177), (24, 183), (29, 188), (38, 192),
        (105, 192), (105, 142), (94, 112), (54, 112), (42, 123)
    ]
    ax.add_patch(Polygon(catcher_section, closed=True, facecolor="#F2F2ED", edgecolor="#112431", lw=3))
    ax.add_patch(Polygon([(28, 126), (28, 175), (31, 182), (39, 186), (99, 186), (99, 143), (89, 118), (59, 118), (48, 126)], closed=True, facecolor="#F4F1EB", edgecolor="#C9CDCB", lw=1.5))
    ax.plot([22, 24, 29, 38], [177, 183, 188, 192], color="#08777D", lw=6, solid_capstyle="round")
    ax.text(12, 157, "64 mm\nPrallwand", ha="right", va="center", fontsize=11, color="#112431")
    ax.annotate("10 mm Fanghaube\n(max. 43°)", xy=(31, 186), xytext=(-60, 220), arrowprops={"arrowstyle": "->", "color": "#08777D", "lw": 2}, fontsize=12, color="#112431")

    ax.add_patch(Rectangle((105, 142), 7, 43, facecolor="#F2F2ED", edgecolor="#112431", lw=2))
    ax.add_patch(Rectangle((112, 148), 34, 29, facecolor="#D8D8D3", edgecolor="#112431", lw=2))
    ax.text(129, 162.5, "Purge\nWiper", ha="center", va="center", fontsize=11, color="#112431")
    ax.add_patch(FancyArrowPatch((138, 162), (34, 154), connectionstyle="arc3,rad=0.09", arrowstyle="-|>", mutation_scale=20, lw=3, color="#C7AB82"))
    ax.text(76, 169, "Feder-Auswurf", ha="center", color="#8A6A3E", fontsize=11)
    ax.add_patch(FancyArrowPatch((50, 150), (72, 118), connectionstyle="arc3,rad=-0.25", arrowstyle="-|>", mutation_scale=20, lw=3, color="#08777D"))
    ax.add_patch(FancyArrowPatch((74, 109), (74, 91), arrowstyle="-|>", mutation_scale=20, lw=3, color="#08777D"))
    ax.text(82, 104, "Umlenkung nach unten", color="#08777D", fontsize=11, va="center")

    ax.add_patch(Polygon([(8, 0), (142, 0), (139, 77), (11, 77)], closed=True, facecolor="#112431", edgecolor="#08777D", lw=3))
    ax.plot([10, 140], [77, 77], color="#7FD5D3", lw=7, solid_capstyle="round")
    ax.text(75, 38, "separater Behälter", ha="center", va="center", fontsize=15, color="#F2F2ED")

    ax.text(170, 218, "1  Vertikale M3-Messlehre prüfen", fontsize=13, color="#112431")
    ax.annotate("2  Fangkorb am Wischerträger montieren", xy=(110, 165), xytext=(170, 174), arrowprops={"arrowstyle": "->", "color": "#08777D", "lw": 2}, fontsize=13, color="#112431")
    ax.annotate("3  Behälter lose darunterstellen", xy=(139, 73), xytext=(170, 105), arrowprops={"arrowstyle": "->", "color": "#08777D", "lw": 2}, fontsize=13, color="#112431")
    ax.text(20, 232, "68 × 46 mm · hohe massive Trefferseite · Waben nur an den Seiten · Logo × 3", fontsize=12, color="#112431")
    ax.text(8, -12, "keine Verbindung zwischen Fangkorb und Behälter", fontsize=11, color="#112431")
    ax.set_xlim(-78, 400)
    ax.set_ylim(-20, 248)
    ax.set_aspect("equal")
    ax.set_title("R3-Montage- und Fangprinzip", fontsize=20, color="#112431", pad=14)
    ax.axis("off")
    fig.tight_layout()
    path.parent.mkdir(parents=True, exist_ok=True)
...
def save_three_side_preview(path: Path, masks: dict, params: dict):
    ...
    heights = (float(params["front_deflector_height_mm"]), float(params["side_wall_height_mm"]), float(params["side_wall_height_mm"]))
    ...
    for face_index, (ax, label, face_height, face_width) in enumerate(zip(axes, labels, heights, panel_widths)):
        ...
        if face_index:
            for hx in np.arange(-face_width / 2.0 + 4, face_width / 2.0, 8):
                for hz in np.arange(6 + (4 if int((hx + face_width / 2.0) / 8) % 2 else 0), face_height - 3, 7):
                    ax.add_patch(RegularPolygon((hx, hz), 6, radius=4.1, orientation=0, fill=False, edgecolor="#C9CDCB", lw=1.0))
        extent = [-panel_w / 2.0, panel_w / 2.0, logo_bottom, logo_bottom + panel_h]
        ...
        ax.text(0, 5, "massive Prallwand" if face_index == 0 else "Innenhaut + Wabenrippen", ha="center", color="#112431", fontsize=8.5, bbox={"fc": "#F2F2ED", "ec": "none", "alpha": 0.9})
...
def save_catcher_section_preview(path: Path, occupancy: np.ndarray, params: dict):
    from matplotlib.colors import ListedColormap
    from matplotlib.patches import FancyArrowPatch

    pitch = float(params["voxel_pitch_mm"])
    upper_w = float(params["upper_width_mm"])
    upper_d = float(params["upper_depth_mm"])
    plate_t = float(params["mount_plate_thickness_mm"])
    nx, ny, nz = occupancy.shape
    x_extent = (-upper_w / 2.0, -upper_w / 2.0 + nx * pitch)
    y_extent = (-upper_d / 2.0, -upper_d / 2.0 + ny * pitch)
    z_extent = (0.0, nz * pitch)
    cmap = ListedColormap(["#F4F1EB", "#C9CDCB"])
    fig, axes = plt.subplots(1, 3, figsize=(14, 6), facecolor="#F4F1EB")

    center_section = occupancy[nx // 2, :, :].T
    axes[0].imshow(center_section, extent=(*y_extent, *z_extent), origin="lower", interpolation="nearest", cmap=cmap, vmin=0, vmax=1)
    axes[0].add_patch(FancyArrowPatch((18, 48), (-17, 45), connectionstyle="arc3,rad=0.12", arrowstyle="-|>", mutation_scale=18, lw=2.5, color="#C7AB82"))
    axes[0].add_patch(FancyArrowPatch((-14, 42), (0, 16), connectionstyle="arc3,rad=-0.25", arrowstyle="-|>", mutation_scale=18, lw=2.5, color="#08777D"))
    axes[0].annotate("10 mm Haube", xy=(-14, 61), xytext=(-6, 69), arrowprops={"arrowstyle": "->", "color": "#08777D"}, color="#112431", fontsize=10)
    axes[0].text(17, 51, "Wischer", color="#112431", fontsize=10, ha="center")
    axes[0].set_title("Mittelschnitt: Treffer → Fall", color="#112431")
    axes[0].set_xlabel("Y [mm]  (−Y Prallwand · +Y Wischer)")
    axes[0].set_ylabel("Z Höhe [mm]")

    front_projection = np.any(occupancy, axis=1).T
    axes[1].imshow(front_projection, extent=(*x_extent, *z_extent), origin="lower", interpolation="nearest", cmap=cmap, vmin=0, vmax=1)
    axes[1].axhline(float(params["impact_hood_start_z_mm"]), color="#08777D", ls="--", lw=1.5)
    axes[1].text(0, float(params["impact_hood_start_z_mm"]) + 1.5, "Beginn Fanghaube", ha="center", color="#08777D", fontsize=9)
    axes[1].set_title("Prallwand 68 × 64 mm", color="#112431")
    axes[1].set_xlabel("X Displayseite [mm]")
    axes[1].set_ylabel("Z Höhe [mm]")

    plate_projection = occupancy[:, ny - 1, :].T
    axes[2].imshow(plate_projection, extent=(*x_extent, *z_extent), origin="lower", interpolation="nearest", cmap=cmap, vmin=0, vmax=1)
    axes[2].text(0, 46, "2 vertikale\n8 × 4,2 mm\nLanglöcher", ha="center", va="center", color="#112431", fontsize=10)
    axes[2].set_title("Befestigungsplatte", color="#112431")
    axes[2].set_xlabel("X [mm]")
    axes[2].set_ylabel("Z Höhe [mm]")

    for ax in axes:
        ax.set_aspect("equal")
        ax.grid(False)
    fig.suptitle("R3 – exakter Voxelschnitt der generierten Druckgeometrie", fontsize=18, color="#112431")
    fig.tight_layout()
    path.parent.mkdir(parents=True, exist_ok=True)
    fig.savefig(path, dpi=180, bbox_inches="tight", facecolor=fig.get_facecolor(), metadata={"Software": "metriMade deterministic generator"})
    plt.close(fig)
...
def build(root: Path):
    ...
    write_3mf(
        ...
        "metriMade compact impact-hood purge catcher R3",
        [(catcher, 0), (logos["#112431"], 1), (logos["#08777D"], 2), (logos["#7FD5D3"], 3), (logos["#C7AB82"], 4)],
        materials,
    )
    write_3mf(mf_dir / "lower-bin-core.3mf", "freestanding lower bin R3", [(lower_bin, 0)], [("navy", "#112431FF")])
    write_3mf(mf_dir / "mount-fit-gauge-core.3mf", "vertical pair mount fit gauge R3", [(gauge, 0)], [("gauge", "#7FD5D3FF")])
    ...
    save_catcher_section_preview(root / "previews/catcher-r3-section.png", catcher_occupancy, params_catcher)
    ...
    digital_checks = {
        ...
        "mount_slots_are_vertical_pair": catcher_metrics["mount_slot_axis"] == "vertical pair",
        "solid_impact_wall_present": catcher_metrics["impact_wall_solid"],
    ...
    (reports / "source-build-report.json").write_text(json.dumps({
        ...
        "limitations": [
            ...
            "Purge fragment velocity and trajectory are undocumented; the 64 mm impact wall and 10 mm inward hood require three supervised purge cycles before unattended use.",
