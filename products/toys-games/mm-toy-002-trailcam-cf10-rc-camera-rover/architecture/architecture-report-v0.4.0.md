...
| DEC-AXLE-001 | open | Select the axle/differential approach: purchased gearbox/diff unit preferred; printed low-speed gears/axles only with explicit strain justification and bench tests. | design-spec.yaml revision 0.4.0 recommends a purchased gearbox/diff unit when a suitable COTS unit exists; printed gears or axles are restricted to the low-speed stage. | Named candidate gearbox/diff unit with measured envelope, or printed low-speed gear/axle strain justification plus bench test results per design-spec 0.4.0. | component, integration, manufacturing, physical, release |
| DEC-DRIVE-001 | open | Freeze the exact drivetrain set (motor class, ratio, servo size) and wheel/tire size for the printed chassis. | design-spec.yaml revision 0.4.0 lists motor/ratio/servo and wheel/tire size as unresolved; only 1:10 class values exist. | Named motor, reduction ratio, steering servo size and wheel/tire selection with measured envelopes. | component, integration, manufacturing, physical, release |
...
| DRIVETRAIN_COTS | purchased | cots | purchased drivetrain authority: motor, reduction gearbox and diff unit candidate per DEC-AXLE-001/DEC-DRIVE-001; printed low-speed gears/axles only if no suitable purchased unit exists and only with explicit strain justification and bench tests | [-120, -110, -75] → [120, 110, -5] (240 × 220 × 70 mm) | supplier assembly / drivetrain | IF-CHASSIS-DRIVE, IF-DRIVE-CARRIERS |
